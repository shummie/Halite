    ...
    def get_best_moves(self):
        ...
        for border in production_squares:
            ...
            if border[1] >= threshold:
                find_cell = self.attack_cell(border[0], 3)
            if find_cell: 
        ...
        for square in cells_to_consider_moving:
            ...
            if square.strength > (square.production * buildup_multiplier):
                if len(square.moving_here) == 0 and (square.x + square.y) % 2 == self.frame % 2:
                    ...
                    target_map = self.smoothed_value_distance_map + self.distance_map_no_decay[square.x, square.y] * 2
...
def new_first_turns_moves():
    ...
    for square in cells_to_consider_moving:
        if square.strength > (square.production * early_game_buildup_multiplier):
            target_map = game_map.smoothed_value_distance_map[square.x, square.y]
