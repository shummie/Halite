...
class Maps():
    ...
    def time_to_recover(self):
        ...
        self.recover_map[0] += (self.is_owner_map + self.is_enemy_map) * 999
        ...
        while distance <= max_distance:
            ...
            self.recover_map[distance] = numpy.add(self.recover_map[distance - 1], numpy.amax(dir_map, 0))
            distance += 1
...
class EarlyGameEvaluator(Evaluator):
    ...
    def update(self):
        self.value_map = numpy.copy(self.game_map.maps[self.game_map.frame].future_value_map)
...
class MidGameEvaluator(Evaluator):
    ...
    def update(self):
        ...
        # Frames back = 5
        lookback = 5
        self.value_map += self.game_map.maps[self.game_map.frame].future_value_map[7] * future_value_factor
        ...
        for square in self.game_map:
            if self.game_map.maps[self.game_map.frame].border_map[square.x, square.y] and self.game_map.evaluators[self.phase].value_map[square.x, square.y] > 0:
                if self.game_map.maps[self.game_map.frame].influence_enemy_territory_map[6, square.x, square.y] == 0:
                    ...
                    production_targets.append((square, self.game_map.evaluators[0].value_map[9, square.x, square.y]))
    ...
    def get_moves(self):
        ...
        for square in cells_to_consider_moving:
            if square.strength > (square.production * early_game_buildup_multiplier):
                ...
                if len(square.moving_here) > 0:
                    continue
                if self.game_map.get_distance(square, border_squares[0][0]) > 3:
                    square.move_to_target(border_squares[0][0], True)
                else:
                    if square.strength > border_squares[0][0].strength:
                        square.move_to_target(border_squares[0][0], False)
