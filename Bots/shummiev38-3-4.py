...
class Game:
    ...
    def create_dijkstra_maps(self):
        def get_cost_recov(cellnum):
            ...
            #return self.strength_map_1[x, y]
    ...
    def update_configs(self):
        ...
        self.buildup_multiplier = np.ones((self.width, self.height)) * 5
...
#            self.distance_from_combat_zone += (1-self.is_owned_map) * 999
    ...
    def update_recover_maps(self):
        ...
        #self.prod_over_str_map[0] = np.divide(self.production_map, self.strength_map_01) * (self.is_neutral_map - self.combat_zone_map)
        self.prod_over_str_map[0] = np.divide(self.production_map * 1.1, new_str_map) * (self.is_neutral_map - self.combat_zone_map)
        self.prod_over_str_map[0][self.prod_over_str_map[0] > (self.max_turns - self.frame) * 0.25] = 0
        ...
        for distance in range(1, max_distance + 1):
            ...
            self.prod_over_str_map[distance] = self.prod_over_str_map[distance] / (1 + 4 * 1)
    ...
    def update_value_production_map(self):
        ...
        recover_threshold = turns_left * 0.40
        ...
        if self.global_max_square == None or not self.is_neutral_map[self.global_max_square.x, self.global_max_square.y]:
        ...
        avg_cost_to_global -= 5 # Testing various values to weight towards global max.
        self.value_production_map[self.border_square_closest_to_global.x, self.border_square_closest_to_global.y] = min(self.total_avg_cost_to_global, avg_cost_to_global, self.value_production_map[self.border_square_closest_to_global.x, self.border_square_closest_to_global.y])
        ...
        for square in idle_squares:
            if square.strength > square.production * self.buildup_multiplier[square.x, square.y] and square.move == -1 and square.moving_here == []:
                if self.percent_owned > 0.65:
                    ...
                    value_map -= self.controlled_production_influence_map[7, square.x, square.y] * 5 * self.combat_zone_map 
