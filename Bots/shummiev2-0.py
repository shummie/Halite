...
def consolidate_strength():
    ...
    for border_square in border_squares:
        ...
        neighbors = game_map.neighbors(border_square[0])
        surrounding_strength = 0
        friendly_neighbors = [n for n in neighbors if n.owner == my_id]
        ...
        # Case 2: Ok, the surrounding squares aren't enough. Can we gather strength in an adjacent friendly square 1 space away, so that this can be captured next turn?
            ...
            for f in friendly_neighbors:
                ...
                f_neighbors = game_map.neighbors(f)
                f_neighbors = [x for x in f_neighbors if (x.owner == my_id and game_map.move_map[x.y, x.x] == STILL)]
                if sum(x.strength for x in f_neighbors) > needed_strength:
