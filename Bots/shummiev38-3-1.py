...
#            self.distance_from_combat_zone += (1-self.is_owned_map) * 999
    ...
    def update_value_production_map(self):
        ...
        if self.global_max_square == None:
            ...
            self.value_production_map[border_square_closest_to_global.x, border_square_closest_to_global.y] = min(self.total_avg_cost_to_global, avg_cost_to_global, self.value_production_map[border_square_closest_to_global.x, border_square_closest_to_global.y])
        else:
            recover_map = 1 / np.maximum(self.prod_over_str_avg_map, 0.01)
            recover_map += self.distance_from_owned ** 0.05
            tx, ty = np.unravel_index(recover_map.argmin(), (self.width, self.height))
