...
class Game:
    ...
    def create_dijkstra_maps(self):
        ...
        def get_cost_recov(cellnum):
            x = cellnum // h
            y = cellnum % h
            return max(1, self.strength_map_1[x, y])
    ...
    def update_configs(self):
        ...
        if self.my_production_sum / self.next_highest_production_sum > 1.1:
            self.buildup_multiplier += 1
        ...
        # if np.sum(self.is_owned_map) > 4:
        #     self.buildup_multiplier += 1

        self.buildup_multiplier += self.is_owned_map * (self.distance_from_border > 1)
    ...
    def update_value_production_map(self):
        ...
        self.base_value_map = np.divide(self.production_map_01 ** 1.1, self.strength_map_1) * (self.is_neutral_map - self.combat_zone_map)
        ...
        total_p = np.sum(self.production_map * self.is_owned_map)
        ...
        for g in global_targets:
            if self.base_value_map[g.x, g.y] > 0.02:
                ...
                self.global_border_map[tx, ty] += self.base_value_map[g.x, g.y] / (self.dij_recov_distance_map[g.x, g.y, tx, ty] / total_p)
    ...
    def get_moves_other(self):
        ...
        for square in idle_squares:
            ...
            if square.strength > square.production * self.buildup_multiplier[square.x, square.y] and square.move == -1 and square.moving_here == []:
                if self.percent_owned > 0.55:
                    self.find_nearest_combat(square)
                else:
    ...
    def attack_cell(self, target, max_cells_out, min_cells_out=1):
        ...
        while cells_out <= max_cells_out:
            ...
            if target.strength == 0 or target.production >= 8:  # or self.phase == 0:
            ...
            if available_strength + available_production > target.strength + 0:
                ...
                # if nstr == 0:
                #     nstr = 1
                # self.value_production_map[target.x, target.y] = 1 / max(nprod / nstr + self.global_border_map[target.x, target.y], 0.001)
                return True
            else:
                cells_out += 1
        return False
    ...
    def find_nearest_combat(self, square):
        current_distance = self.distance_from_combat_zone[square.x, square.y]
        # Todo, minor optimization by moving to lower production square if possible.
        for n in square.neighbors:
            if self.is_owned_map[n.x, n.y]:
                if self.distance_from_combat_zone[n.x, n.y] < current_distance:
                    success = self.move_square_to_target(square, n, True)
                    if success:
                        break
