...
class Game:
    ...
    def set_configs(self):
        ...
        self.production_cells_out = 8
        self.mid_game_trigger_square = None
...
#        start = time.time()
        if game.frame == 1:
            self.create_beginning_game_stats()
        self.update_maps()
#        end = time.time()
#        logging.debug("update_maps Frame: " + str(game.frame) + " : " + str(end - start))
        ...
        if self.mid_game_trigger_square == None:
            recover_map = 1 / np.maximum(self.prod_over_str_avg_map, 0.01)
            recover_map += self.distance_from_owned ** 0.25
            tx, ty = np.unravel_index(recover_map.argmin(), (self.width, self.height))
            self.mid_game_trigger_square = self.squares[tx, ty]
        
        if game.phase == 0:
            if np.sum(self.combat_zone_map) > 0: #or np.sum(self.is_owned_map) > (10*(self.width * self.height)**.5) / ((self.starting_player_count**0.5) * 5):
                game.phase = 1
            if self.border_map[self.mid_game_trigger_square.x, self.mid_game_trigger_square.y]:
                game.phase = 1
...
#        start = time.time()
        if self.phase == 0:
            ...
            self.early_game_get_moves_other()
        elif self.phase == 1:
...
#                return
    ...
    def early_game_production(self):
        ...
        potential_targets = [(self.squares[c[0], c[1]], recover_map[c[0], c[1]], 1) for c in potential_targets_indices]

        potential_targets = []
        early_production_cells_out = 12
        for c in potential_targets_indices:
            target = self.squares[c[0], c[1]]
            value = self.recover_wtd_map[c[0], c[1]]
            cells_out = 1
            while cells_out <= early_production_cells_out:
                potential_targets.append((target, value, cells_out))
                cells_out += 1
        ...
        # Keep only the top 80ile?
        #potential_targets = potential_targets[0:int(len(potential_targets) * .25)]

        
        best_target_value = potential_targets[0][1]
        # anything with X of the best_value target should be considered. Let's set this to 4 right now.
        while len(potential_targets) > 0 and potential_targets[0][1] <= (best_target_value + 1):
            target = potential_targets.pop(0)
            success = self.attack_cell(target[0], target[2], target[2])
#            if success and target[2] < self.production_cells_out:
            if success and target[2] < early_production_cells_out:
                potential_targets = list(filter(lambda sq: sq[0] != target[0], potential_targets))
                            
    def early_game_get_moves_other(self):
        # Tries to move to 
        recover_map = 1 / np.maximum(self.prod_over_str_avg_map, 0.01)
        ...
        for square in idle_squares:
            if square.strength > square.production * self.buildup_multiplier[square.x, square.y] and square.move == -1:
                ...
                value_map = np.multiply(recover_map, self.border_map)
                value_map[value_map == 0] = 9999
                tx, ty = np.unravel_index(value_map.argmin(), (self.width, self.height))
                logging.debug("tx/ty" + str(tx) + "/" + str(ty) + " argmin: " + str(value_map[tx, ty]))
...
#        best_target_value = potential_targets[0][1]
    ...
    def create_beginning_game_stats(self):       
        self.beginning_average_recovery = np.sum(self.strength_map) / np.sum(self.production_map)
