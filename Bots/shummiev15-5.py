...
production_self_factor = 1
production_neutral_factor = 2
production_enemy_factor = 1
production_influence_factor = 4 # Sample values are around 50-80
production_square_influence_factor = 4
prod_over_str_influence_factor = 4 # Sample values are around 0.5 - 1.0
...
late_game_prod_over_str_neutral_factor = 0
...
class Maps():
    ...
    def update_heuristic_map(self):
        ...
        if self.game_map.phase <= 1:
            ...
            self.heuristic_map += numpy.multiply(self.production_map, self.is_owner_map[self.game_map.my_id]) * production_self_factor
            ...
            self.heuristic_map += self.influence_enemy_strength_map[1] * enemy_strength_1_influence_factor
            self.heuristic_map += self.influence_enemy_strength_map[2] * enemy_strength_2_influence_factor
            self.heuristic_map += self.influence_enemy_strength_map[3] * enemy_strength_3_influence_factor
    
            self.heuristic_map += self.influence_enemy_territory_map[1] * enemy_territory_1
            self.heuristic_map += self.influence_enemy_territory_map[2] * enemy_territory_2
            self.heuristic_map += self.influence_enemy_territory_map[3] * enemy_territory_3
...
def heuristic(cell, source = None):
    ...
    if cell.strength / max(cell.production, 0.01) > game_map.turns_left * 0.65:
        return 0
...
def first_turns_heuristic():
    ...
    for square in game_map:
        if square.is_npc_border():
            border_squares.append((square, game_map.maps[game_map.frame].recover_map[game_map.early_game_squares_out_search, square.x, square.y]))
    ...
    for square in cells_to_consider_moving:
        if square.strength > (square.production * early_game_buildup_multiplier):
            ...
            #else:
            #if game_map.get_distance(square, border_squares[0][0]) > 1:
            #    square.move_to_target(border_squares[0][0], True)
                
            square_cost = game_map.maps[game_map.frame].recover_map[6] + game_map.distance_map[square.x, square.y, :, :] ** 1.4
            square_cost = square_cost * game_map.maps[game_map.frame].border_map
            square_cost += (game_map.maps[game_map.frame].is_owner_map[game_map.my_id] + game_map.maps[game_map.frame].is_enemy_map) * 999
            tx, ty = numpy.unravel_index(square_cost.argmin(), (game_map.width, game_map.height))
            #border_squares.sort(key = lambda x: self.game_map.get_distance(square, x[0]))
            target = game_map.squares[tx, ty]

            if game_map.maps[game_map.frame].border_map[square.x, square.y]:
                square.move_to_target(target, False)

            elif game_map.get_distance(square, target) > 4:
                square.move_to_target(target, True)
            else:
                if game_map.get_distance(square, target) == 1 and square.strength > target.strength:
                    square.move_to_target(target, False)
        
def create_djik_early():
    # Cell [x, y] corresponds to x * self.width + y in the grid below.
    dj_graph = numpy.zeros((game_map.width * game_map.height, game_map.width * game_map.height))
    # Create the distance graph

    for x in range(game_map.width):
        for y in range(game_map.height):
            coord = x * game_map.width + y
            dj_graph[coord, ((x + 1) % game_map.width) * game_map.width + ((y + 0) % game_map.height)] = 1
            dj_graph[coord, ((x - 1) % game_map.width) * game_map.width + ((y + 0) % game_map.height)] = 1
            dj_graph[coord, ((x + 0) % game_map.width) * game_map.width + ((y + 1) % game_map.height)] = 1
            dj_graph[coord, ((x + 0) % game_map.width) * game_map.width + ((y - 1) % game_map.height)] = 1
