...
class GameMap:
    ...
    def update(self):
        ...
        if self.phase == 0 and numpy.sum(self.maps.is_owner_map[self.my_id]) > (10*(self.width * self.height)**.5) / ((self.starting_player_count**0.5) * 15):
            self.phase = 1
...
class EarlyGameEvaluator(Evaluator):
    ...
    def update(self):
        self.value_map = numpy.copy(self.game_map.maps.future_value_map)
...
class MidGameEvaluator(Evaluator):
    ...
    def update(self):
        ...
        self.value_map += self.game_map.maps.future_value_map_smooth[9] * future_value_factor
...
class LateGameEvaluator(Evaluator):
    ...
    def update(self):
        ...
        self.value_map += self.game_map.maps.future_value_map_smooth[9] * late_game_future_value_factor
        #self.value_map += self.game_map.maps.future_value_map[9] * late_game_future_value_factor
        
        # The value of any cell we own is 0.
        self.value_map = numpy.multiply(self.value_map, 1 - self.game_map.maps.is_owner_map[self.game_map.my_id])
        
        # If this cell is neutral AND bordered by hostile, let's not attack it.
        self.value_map = numpy.multiply(self.value_map, 1 - numpy.multiply(self.game_map.maps.enemy_border_map, (self.game_map.maps.strength_map > 0) * 1))
        #self.value_map = numpy.multiply(self.value_map, numpy.multiply(self.game_map.maps.strength_map != 0, 1 - numpy.multiply(self.game_map.maps.is_owner_map[0], self.game_map.maps.enemy_border_map)))
    ...
    def get_moves(self):
        ...
        for square in self.game_map:
            ...
            if self.game_map.maps.border_map[square.x, square.y]:
                ...
                if self.game_map.maps.influence_enemy_territory_map[2, square.x, square.y] > 0:
...
class MidTacticalAI(TacticalAI):
    ...
    def get_moves(self):
        ...
        for square in self.game_map:
            if self.game_map.maps.border_map[square.x, square.y] and self.game_map.evaluators[self.phase].value_map[square.x, square.y] > 0:
    ...
    def get_moves(self):
        ...
        for square in cells_to_consider_moving:
            if square.strength > (square.production * buildup_multiplier):
                #tx, ty = numpy.unravel_index(numpy.divide(numpy.multiply(self.game_map.evaluators[self.phase].value_map, self.game_map.maps.border_map), self.game_map.maps.distance_map[square.x, square.y, :, :]).argmax(), (self.game_map.width, self.game_map.height))
                tx, ty = numpy.unravel_index(numpy.divide(numpy.multiply(self.game_map.maps.future_value_map_smooth[5], self.game_map.maps.border_map), self.game_map.maps.distance_map[square.x, square.y, :, :]).argmax(), (self.game_map.width, self.game_map.height))
                
                target = self.game_map.squares[tx, ty]
                if game_map.get_distance(square, target) == 1:
                    square.move_to_target(target, False)
                square.move_to_target(target, True, True)
