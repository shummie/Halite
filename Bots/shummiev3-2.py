...
def get_move(square, buildup_multiplier = 9):
    ...
    if len(targets) > 0:
        ...
        for t in targets:
            if t[0].strength < square.strength:
                return game_map.move_to_target(square, t[0])
...
def consolidate_strength():
    ...
    for border_square in border_squares:
        ...
        # friendly_neighbors = [n for n in game_map.neighbors(border_square[0]) if n.owner == my_id]
        ...
        if available_strength > border_square[0].strength:
            ...
            for f in friendly_neighbors:
                if (game_map.move_map[f.y, f.x] == STILL) and (attacking_strength < border_square[0].strength):
                    ...
                    game_map.move_to_target(f, border_square[0])
...
while True:
    ...
    elif percent_owned > 0.35:
        building_multiplier = 4
    elif percent_owned > 0.25:
        building_multiplier = 5
    elif percent_owned > 0.2:
        building_multiplier = 6
    elif percent_owned > 0.15:
        building_multiplier = 7
    else:
        building_multiplier = 9
    for square in square_move_list:
        get_move(square, building_multiplier)
