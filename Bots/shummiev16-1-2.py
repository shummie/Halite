...
class GameMap:
    ...
    def update(self):
        ...
        if self.phase == 0 and numpy.sum(self.is_owned_map) > (10*(self.width * self.height)**.5) / ((self.starting_player_count**0.5) * 11):
            self.phase = 1
    ...
    def update_heuristic_map_v2(self):
        self.heuristic_map_2 = numpy.zeros((self.width, self.height))
        # We start off with the smoothed recovery map. We'll try 4 squares out for now.

        # Note, unlike the other maps, right now, the recovery map prefers LOW values. 
        self.heuristic_map_2 += self.recover_map_enemy_smooth[4]
        # Currently the map stores an average turns before the cell pays itself off value.
        
    def get_heuristic_2(self, square):
        # The value of the map should actually vary by cell though. A cell that's very far away from the "best" cell should de-value it and instead value cells closer to it a bit higher.
        combat_zone_value = -1
        # Start with the heuristic_map 2
        if self.frame % 5 == 0 or self.heuristic_map_temp == None:
            self.heuristic_map_temp = numpy.copy(self.heuristic_map_2)
            
            # If we are currently bordering an enemy border, give it a small bonus since we prefer attacking/defending our borders if we are nearby.
            self.heuristic_map_temp += self.combat_zone_map * combat_zone_value
            
            # Add the distance map for this square. Squares further away take more time to recover its values
        self.heuristic_map_temp += self.distance_map_no_decay[square.x, square.y] ** 1.25
            
            # We only care about the border cells for our evaluation.
            # self.heuristic_map_temp += (1 - self.border_map) * 999
            
        # We now have a heuristic map adjusted for the square.
        return self.heuristic_map_temp + (1 - self.border_map) * 999
    ...
    def get_best_moves(self):
        ...
        for square in cells_to_consider_moving:
            if square.strength > (square.production * buildup_multiplier):
                square_map = self.get_heuristic_2(square)
                # Move to the most valuable cell.
                tx, ty = numpy.unravel_index(square_map.argmin(), (self.width, self.height))
                if distance_between(square.x, square.y, tx, ty) > 2:
    ...
    def get_best_moves_late_game(self):
        ...
        for square in cells_to_consider_moving:
            if square.strength > (square.production * buildup_multiplier):
                ...
                # Move to the most valuable cell.
                tx, ty = numpy.unravel_index(square_map.argmin(), (self.width, self.height))
                if distance_between(square.x, square.y, tx, ty) > 1:
                    square.move_to_target(self.squares[tx, ty], True)
...
def heuristic(cell, source = None):
    ...
    return self.heuristic_map_2[cell.x, cell.y]
    
    cell_value += production_square_influence_factor * cell.production / max(cell.strength, .11)
    cell_value += game_map.influence_production_map[cell.x, cell.y] * production_influence_factor
