...
class Game:
    ...
    def update_configs(self):
        self.buildup_multiplier = np.minimum(np.maximum(self.production_map, 3), 3)
        # self.buildup_multiplier = np.minimum(np.maximum(self.production_map, 5), 7)
        self.buildup_multiplier = self.buildup_multiplier - np.maximum(self.distance_from_border / 5, 0)
        ...
        if self.percent_owned > 0.1:
            self.combat_radius += self.percent_owned / 0.15
            self.buildup_multiplier += 4 + (self.percent_owned / 0.2)
            self.buildup_multiplier += (self.production_map > 5) * 1
    ...
    def attack_cell(self, target, max_cells_out, min_cells_out=1):
        ...
        while cells_out <= max_cells_out:
            ...
            if np.sum(self.is_owned_map) <= 10 and self.near_enemy is False:
                ...
                if target.strength == 0 or target.production >= 10:  # or self.phase == 0:
                    free_squares = self.is_owned_map * (self.move_map == -1)
                else:
