...
class GameMap:
    ...
    def initialize_game(self):
        ...
        self.strength_path_distance_map = self.create_strength_path_distance_map()
    ...
    def create_strength_path_distance_map(self):
        ...
        self.dij_cost, self.dij_route = scipy.sparse.csgraph.dijkstra(dij_costs, return_predecessors = True)
        
        self.dij_distance_map = numpy.zeros((self.width, self.height, self.width, self.height))
        
        for x in range(self.width):
            for y in range(self.height):
                self.dij_distance_map[x, y, :, :] = self.dij_cost[x * self.height + y].reshape((self.width, self.height))
    ...
    def create_smoothed_value_distance_map(self):
        ...
        for x in range(self.width):
            for y in range(self.height):
                smoothed_value = numpy.divide(self.production_map, self.dij_distance_map[x, y])
...
def first_turns_heuristic():
    ...
    for border in border_squares:
        find_cell = False
        if border[1] <= threshold:
            find_cell = game_map.attack_cell(border[0], 8)
        if find_cell: 
            border_squares.remove(border)
