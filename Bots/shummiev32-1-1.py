# IDEA? Can we focus strength to attacking border cells near high enemy production sites?
#==============================================================================
# Imports
#==============================================================================
import functools
...
class Game:
    ...
    def set_configs(self):
        self.buildup_multiplier = np.minimum(np.maximum(self.production_map, 5), 10)
...
#            self.distance_from_combat_zone = np.ones((self.width, self.height)) * 999
    ...
    def update_recover_maps(self):
        ...
        #self.prod_over_str_map[0] = np.divide(self.production_map, self.strength_map_01) * (self.is_neutral_map - self.combat_zone_map)
        self.prod_over_str_map[0] = np.divide(self.production_map * 1.35, new_str_map) * (self.is_neutral_map - self.combat_zone_map)
...
#        best_target_value = potential_targets[0][1]
    ...
    def get_moves_breakthrough(self):
        ...
        for square in potential_squares:
            if self.own_strength_map[4, square.x, square.y] > 1000 and (self.own_strength_map[4, square.x, square.y] > 1.5 * self.enemy_strength_map[4, square.x, square.y]):
                self.attack_cell(square, 1)
        ...
        for square in idle_squares:
            if square.strength > square.production * self.buildup_multiplier[square.x, square.y] and square.move == -1:
                if self.percent_owned > 0.7:
                    ...
                    value_map += self.distance_map_no_decay[square.x, square.y] * 0.9 * self.combat_zone_map 
                    value_map -= self.controlled_production_influence_map[6, square.x, square.y] * self.combat_zone_map 
                    ...
                    if self.combat_zone_map[tx, ty]:
                        if self.distance_between(square, target) > 15:
