...
class Game:
    ...
    def get_prod_moves(self):
        ...
        for s in idle_squares:
            value_map = (self.value_map + self.distance_map_no_decay[s.x, s.y] * 1.4) * self.border_map
            value_map[np.nonzero(self.combat_zone_map)] = 9999
            value_map[value_map == 0] = 9999
            
            tx, ty = np.unravel_index(value_map.argmin(), (self.w, self.h))
            t = self.squares[tx, ty]      
            self.attack_cell(t, 3)               
