...
def heuristic(cell, source = None):
    ...
    if len(square_move_list) < 2 and game_map.frame < 20 and source != None:
        ...
        max_wait_turns = min(n.strength / (max(1, n.production + source.production)) for n in source_npc_neighbors) # Don't wait longer than the # of turns it takes to capture a cell from scratch
        ...
        max_game_turns_left = math.floor(10 * (game_map.width * game_map.height) ** 0.5 - game_map.frame)
        estimated_turns_production = max(0.20 * max_game_turns_left / game_map.starting_player_count - turns_to_capture, 0)
