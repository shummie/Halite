...
class Game:
    ...
    def get_moves(self):
        ...
        self.get_combat_moves()
        self.get_inner_moves()
    ...
    def get_combat_moves(self):
        ...
        for sq in combat_zone_squares:
            if (sq.strength == 0 and self.combat_zone_map[sq.x, sq.y] == 0):
                    # These are squares which should be captured by the lowest strength piece since higher strength pieces should be attacking.
                    n = [x for x in sq.neighbors if x.owner == self.my_id and x.strength > 0]
                    n.sort(key=lambda x: x.strength)
                    self.move_square_to_target_simple(n[0], sq, False)
            elif (sq.x + sq.y) % 2 == game.frame % 2:
            ...
            if (sq.strength > 0) and (combat_distance_matrix[sq.x, sq.y] == 1) and (sq.move == -1 or sq.move == STILL):
                ...
                if not success:
                    for t in targets:
                        ...
                        if success:
                            break
            elif (sq.strength > (sq.production * (self.buildup_multiplier[sq.x, sq.y] + self.distance_from_combat_zone[sq.x, sq.y]))) and ((sq.x + sq.y) % 2 == self.frame % 2) and sq.move == -1 and sq.moving_here == []:
                self.move_towards_map_old(sq, combat_distance_matrix)
            else:
                if combat_distance_matrix[sq.x, sq.y] > 1:
                    self.make_move(sq, STILL, None)

    def get_inner_moves(self):
        ...
        for s in idle_squares:
            if s.strength > 255:
                ...
                if self.distance_from_border[s.x, s.y] == 1:
                    targets = [x for x in s.neighbors if (x.owner == 0 and self.production_map[x.x, x.y] > 3)]
            ...
            if s.strength > (s.production * self.buildup_multiplier[s.x, s.y]) and s.moving_here == [] and (((s.x + s.y) % 2) == (game.frame % 2)):
                ...
                if np.sum(self.is_owned_map) > 120:
                    ...
                    d_map[d_map == -1] = 9999
                value_map = (self.value_production_map + d_map * 2) * (self.border_map - self.combat_zone_map)
                # value_map[np.nonzero(self.combat_zone_map)] = avg_border_val * 10
                ...
                if (self.enemy_strength_map[3, s.x, s.y] > 0) and (((s.x + s.y) % 2) != (game.frame % 2)):
                        self.make_move(s, STILL, None)
                elif self.combat_zone_map[tx, ty]:
                    if max(self.width, self.height) > 44 and self.distance_between(s, t) > 10:
                        self.find_nearest_non_owned_border(s)
                    elif self.distance_between(s, t) > 14:
                ...
                #     self.value_production_map[t.x, t.y] += 2
                # else:
                #     self.value_production_map[t.x, t.y] += s.strength / 200
