...
class Game:
    ...
    def create_dijkstra_maps(self):
        h, w = self.h, self.w
        cost_recov_map = (self.strength_map_1 - 0.5) / self.production_map_01
            ...
            if (square.strength > 0) and (combat_distance_matrix[square.x, square.y] == 1) and (square.move == -1 or square.move == STILL):
                ...
                if len(possible_moves) > 0:
                    ...
                    for t in possible_moves:
                        if t.move == -1 or t.move == 4 or t.strength < t.production * 2:
        ...
        while cells_out <= max_cells_out:
            ...
            if available_strength + available_production > target.strength + 0:
                ...
                if needed_strength_from_movers > 0:
                    ...
                    if self.combat_zone_map[target.x, target.y] == 0:
    ...
    def check_parity(self):
        indices = np.transpose(np.nonzero((self.is_owned_map * (self.enemy_strength_map[3] < 1))))
        ...
        for s in squares:
            ...
            elif (self.enemy_strength_map[2, s.x, s.y] >= 1) and (s.move == STILL or s.move == -1):
