    ...
    if cell.owner == 0:
        if len(game_map.get_all_moving_to(cell)) > 0: # If someone is moving here, then we probably don't need to since they should have enough strength.
            ...
            for n in cell_npc_neighbors:
                ...
                n_cells += 1
            value += 0.3 * n_values    # Give partial weight to neighboring cells. Will then attempt to capture cells that have neighbors over cells that don't...
            return value
    else:
        total_damage = 0
