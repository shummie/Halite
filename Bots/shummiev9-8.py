...
botname = "shummie v9.8"

production_decay = 0.3
...
production_influence_factor = 0.2 # Sample values are around 50-80
production_square_influence_factor = 60
...
border_distance_decay_factor = 1.4
border_target_percentile = .65
...
def first_turns_heuristic():
    ...
    npc_str_map = numpy.maximum(numpy.multiply(game_map.strength_map, game_map.owner_map[0]), 0.001)
    npc_prod_map = numpy.maximum(numpy.multiply(game_map.production_map, game_map.owner_map[0]), 0.001)
    
    recover_map = numpy.divide(npc_prod_map, npc_str_map)
    total_map = numpy.copy(recover_map)
    # How mnay turns do we want to look outwards?
    cells_out = 4
    
    count = 0
    while count < cells_out:
        r1 = numpy.add(recover_map, roll_x(recover_map, 1))
        r2 = numpy.add(recover_map, roll_x(recover_map, -1))
        r3 = numpy.add(recover_map, roll_y(recover_map, 1))
        r4 = numpy.add(recover_map, roll_y(recover_map, -1))
        recover_map = numpy.maximum.reduce([r1,r2 ,r3 ,r4 ])
        count += 1
    
    game_map.border_square_list.sort(key = lambda x: recover_map[x.x, x.y], reverse = True)
    # Assume we are always going to go for the first cell
    find_cell = True
    while find_cell:
        find_cell = game_map.attack_cell(game_map.border_square_list[0], 7)
        
def first_turns_heuristic2():
    
    npc_str_map = numpy.maximum(numpy.multiply(game_map.strength_map, game_map.owner_map[0]), 0.001)
    npc_prod_map = numpy.maximum(numpy.multiply(game_map.production_map, game_map.owner_map[0]), 0.001)
    
    gain_map = numpy.divide(npc_prod_map, npc_str_map)
    
    gain_map = spread_n(gain_map, 10, 1)
    
    game_map.border_square_list.sort(key = lambda x: gain_map[x.x, x.y], reverse = True)
    # Assume we are always going to go for the first cell
    
    find_cell = True
    index = 0
    initial_val = gain_map[game_map.border_square_list[0].x, game_map.border_square_list[0].y]
    while find_cell and gain_map[game_map.border_square_list[0].x, game_map.border_square_list[0].y] >= 0.8 * initial_val:
        find_cell = game_map.attack_cell(game_map.border_square_list[index], 7)
        index += 1
...
def first_turns_heuristic3():
    ...
    find_cell = True
    index = 0
    threshold = border_squares[0][1] * 0.8
    while find_cell:
    ...
    for square in cells_to_consider_moving:
        if square.strength > square.production * 6:
...
def game_loop():
    ...
    if numpy.sum(game_map.is_owner_map[game_map.my_id]) < (10*(game_map.width * game_map.height)**.5) / 30 and game_map.frame < (10*(game_map.width * game_map.height)**.5) / 5:
