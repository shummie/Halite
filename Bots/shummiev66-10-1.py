...
class Game:
    ...
    def get_moves_attack(self):
        ...
        for square in combat_squares:
            ...
            elif ((square.strength > (square.production * (self.buildup_multiplier[square.x, square.y] + 2))) or square.strength > 250) and self.distance_from_combat_zone[square.x, square.y] <= 3 and (square.parity == self.parity) and square.move == -1 and square.moving_here == []:
                self.move_towards_map_old(square, combat_distance_matrix)
            elif ((square.strength > (square.production * (self.buildup_multiplier[square.x, square.y] + 2))) or square.strength > 250) and self.distance_from_combat_zone[square.x, square.y] > 3 and square.move == -1 and square.moving_here == []:
                self.move_towards_map_old(square, combat_distance_matrix)
            else:
        ...
        for sq in idle_squares:
            ...
            if sq.strength > sq.production * self.buildup_multiplier[sq.x, sq.y] and sq.move == -1 and sq.moving_here == []:
                if np.sum(self.combat_zone_map) == 0:
                    ...
                    if self.distance_from_combat_zone[sq.x, sq.y] < 3 and sq.parity != game.parity:
                        continue
    ...
    def check_attack_pattern(self):
        one_away_squares = [self.squares[c[0], c[1]] for c in np.transpose(np.nonzero(self.distance_from_combat_zone == 1))]
        one_away_squares = [x for x in one_away_squares if x.owner == self.my_id and x.strength > 0]
        for sq in one_away_squares:
            # Check in radius 2 if staying still could result in overkill.
            # If there are enemy squares in the same parity as us, staying still might result in overkill (game theory)
            check_dir = get_all_d_away(2)
            count = 0
            e_str = 0
            for dx, dy in check_dir:
                ...
                if t.owner != self.my_id and t.owner != 0 and t.strength > 10:
                    count += 1
                    e_str += t.strength
            if count >= 2 and sq.strength < e_str:
                self.make_move(sq, STILL, None)
