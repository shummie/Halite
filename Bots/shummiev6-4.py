...
# -- Move to border. Can we detect which is the best border to reinforce? We should try to head towards high production sites. If we're being attacked, should we reinforce more?
...
# Version 6: Added code to look at turn 1 specifically. Is it worth delaying the capture of a cell for a more future benefit?
# -- Also fixed heuristic function to work with the strength consolidation and multi-attack
...
def heuristic(cell, source = None):
    ...
    # A very early game heuristic which tries to maximize early game production:
    #if not game_map.enemy_within_n_squares(cell, 10) and percent_owned < 0.1 and source != None:
    if len(square_move_list) < 2 and game_map.frame < 20 and source != None:
        # If there is no enemy within 5 squares of this, use the early game heuristic
        # Don't need to do any attack value calculations since no enemy cell is nearby.
        
        # Sees if it's worth waiting a few turns to capture a higher production cell.
        # Look back at the original source (This is duplicating code so hopefully it's not a big deal since it's run for every neighbor of the source)
        source_npc_neighbors = [x for x in game_map.neighbors(source) if x.owner == 0]
        max_wait_turns = min(6, min(n.strength / (max(1, n.production + source.production)) for n in source_npc_neighbors)) # Don't wait longer than the # of turns it takes to capture a cell from scratch
        # if it takes longer to capture this cell than max_wait_turns, this cell is worthless to us
        if (cell.strength - source.strength) / max(0.01, (cell.production + source.production)) > max_wait_turns:
            return 0
        
        # The value of the cell is then determined by how many turns of production we can get out of it and the source cell
        turns_to_capture = math.ceil(max((cell.strength + 1 - source.strength) / (max(source.production, 1)), 0))
        # How many turns of production can we get out of this?
        max_game_turns_left = math.floor(20 * (game_map.width * game_map.height) ** 0.5 - game_map.frame)
        estimated_turns_production = max(0.075 * max_game_turns_left / game_map.starting_player_count - turns_to_capture, 0)
        
        total_production = (cell.production + source.production) * estimated_turns_production + max(0, cell.strength - source.strength)
        
        # Ignoring neighboring cell value for now.
        
        if turns_to_capture == 0:
            # We can capture this cell immediately, let's see if we can capture any neighboring cells immediately
            remaining_str = cell.strength + 1 - source.strength
            target = cell
            while remaining_str > 0:
                npc_neighbors = source_npc_neighbors = [x for x in game_map.neighbors(target) if x.owner == 0]
                # Use a greedy algorithm here.
                next_targets = []
                for n in npc_neighbors:
                    if n.strength < remaining_str:
                        next_targets.append(n, n.production / max(1, n.strength))
                next_targets.sort(key = lambda x: x[1], reverse = True)
                if len(next_targets) > 1:
                    target = next_targets[0][0]
                    remaining_str -= target.strength
                    total_production += (target.production) * estimated_turns_production - target.strength
                else:
                    remaining_str = -1
...
def get_move(square, buildup_multiplier = 5):
    ...
    if len(square_move_list) < 2 and game_map.frame < 20:
        if len(targets) > 0:
            targets = [targets[0]]
...
class GameMap:
    ...
    def enemy_within_n_squares(self, square, n=1):
        for dx in range(-n, n+1):
            for dy in range(-n, n+1):
                if abs(dx) + abs(dy) <= n:
                    owner_id = self.owner_map[((square.y + dy) % self.height), ((square.x + dx) % self.width)]
                    if owner_id != 0 and owner_id != self.my_id:
                        return True
        return False
...
while True:
    ...
    # Do stuff
    if len(square_move_list) > 1 or game_map.frame > 20:
