...
class GameMap:
    ...
    def update(self):
        ...
        if self.phase == 0 and numpy.sum(self.maps[self.frame].is_owner_map[self.my_id]) > (10*(self.width * self.height)**.5) / ((self.starting_player_count**0.5) * 10):
            self.phase = 1
        if self.phase == 1 and numpy.sum(self.maps[self.frame].is_owner_map[0]) < (self.width * self.height * 0.4):
            self.phase = 2
            
        self.create_maps()  # Until we remove all of them to the Maps class
    ...
    def create_future_map(self, max_distance = 6):
        ...
        while distance <= max_distance:
            ...
            dir_map[0] = roll_xy(future_value_map, 0, 1)
            dir_map[1] = roll_xy(future_value_map, 0, -1)
            dir_map[2] = roll_xy(future_value_map, 1, 0)
            dir_map[3] = roll_xy(future_value_map, -1, 0)
        
            future_value_map = numpy.add(future_value_map, numpy.amax(dir_map, 0))
            distance += 1
        
        self.future_value_map = future_value_map
    ...
    def get_best_moves(self):
        ...
        for square in itertools.chain.from_iterable(self.squares):
            if square.is_npc_border():
                if self.maps[self.frame].influence_enemy_territory_map[6, square.x, square.y] == 0:
        ...
        for square in cells_to_consider_moving:
            if square.is_border() == True:
                ...
                if len(targets) > 0:
                    ...
                    if heuristic(targets[0]) != 0:
                        square.move_to_target(targets[0], False, False)
                    else:
                        # Move this square away from this border.
                        self.go_to_border3(square)
            elif square.strength > (square.production * buildup_multiplier):
                if square.moving_here == []:
                #self.go_to_border3(square)
            ...
            if available_strength + available_production > target.strength:
                ...
                if needed_strength_from_movers > 0:
                    ...
                    for square in moving_cells:
                        if cells_out == 1:
                            square.move_to_target(target, False, False)
                        else:
                            square.move_to_target(target, True, False)
    ...
    def go_to_border(self, square):
        ...
        self_index = square.x * self.height + square.y
        inner_border_squares = []
        for x in range(0, self.width):
            for y in range(0, self.height):
                if self.maps[self.frame].inner_border_map[x, y] == 1:
                    inner_border_squares.append(x * self.height + y)
        # These are a list of inner border squares. find the one that has the smallest path
        target_index = -1
        length = 999
        for sq in inner_border_squares:
            path_length = self.maps[self.frame].dj_path[self_index, sq]
            if path_length < length:
                length = path_length
                target_index = self.maps[self.frame].dj_route[sq, self_index]
        return square.move_to_target(self.squares[target_index // self.height, target_index % self.height], True)
        
    def go_to_border3(self, square):
        # Going to do a simple search for the closest border then determine which of the 4 directions we should go
        ...
        for x in range(0, self.width):
            for y in range(0, self.height):
                if self.maps[self.frame].border_map[x, y]:
                    dj_path, dj_route = scipy.sparse.csgraph.dijkstra(self.maps[self.frame].dj_graph, indices = [self_index, x * self.height + y], return_predecessors = True)
                    inner_border_squares.append((x * self.height + y, heuristic(self.squares[x, y]) / dj_path[1, x * self.height + y]))
        # These are a list of inner border squares. find the one that has the smallest path
        inner_border_squares.sort(key = lambda x: x[1], reverse = True)

        dj_path, dj_route = scipy.sparse.csgraph.dijkstra(self.maps[self.frame].dj_graph, indices = [self_index, inner_border_squares[0][0]], return_predecessors = True)        
        
        target_index = dj_route[1, self_index]
        return square.move_to_target(self.squares[target_index // self.height, target_index % self.height], True)
    ...
    def find_nearest_non_npc_enemy_direction(self, square):
        ...
        while not success and index < len(dir_distance):
            success = square.move_to_target(dir_distance[index][2], False, False)
            index += 1
        if not success:
            if len(not_dir) > 0:
                success = square.move_to_target(not_dir[0][2], False, False)
...
class Square:
    ...
    def move_to_target(self, destination, through_friendly, dijkstra = True):
        
        if not dijkstra:
            return self.move_to_target_old(destination, through_friendly)

        # The problem with dijkstra is that it only allows a single path. If we need to reroute, we may as well default to the old movement then.
        source_index = self.x * self.game_map.height + self.y
        dest_index = destination.x * self.game_map.height + destination.y
        dj_path, dj_route = scipy.sparse.csgraph.dijkstra(self.game_map.maps[self.game_map.frame].dj_graph, indices = [source_index, dest_index], return_predecessors = True)
        target_index = dj_route[1, source_index]
        if target_index == -9999:
            # We're moving to the same square.
            return False
        tx = target_index // self.game_map.height
        ty = target_index % self.game_map.height
        dx = tx - self.x
        if dx == self.game_map.width - 1:
            dx = -1
        elif dx == 1 - self.game_map.width:
            dx = 1
        dy = ty - self.y
        if dy == self.game_map.height - 1:
            dy = -1
        elif dy == 1 - self.game_map.height:
            dy = 1
        
        # (0, -1) NORTH, (0, 1) SOUTH, (-1, 0) WEST, (1, 0) EAST
        #logging.debug("Source x:" + str(self.x) + " source y: " + str(self.y) + " source num: " + str(source_index))
        #logging.debug("dest x:" + str(destination.x) + " dest y: " + str(destination.y) + " dest num: " + str(dest_index))
        #logging.debug("target x:" + str(tx) + " target y: " + str(ty) + " target num: " + str(target_index))
        if (dx, dy) == (0, 1): 
            direction = SOUTH
        elif (dx, dy) == (0, -1):
            direction = NORTH
        ...
        target = self.game_map.get_target(self, direction)
        
        if through_friendly:
            if target.owner != self.game_map.my_id:
                return self.move_to_target_old(destination, through_friendly)
        
        # Can we move to this square?
        future_strength = self.strength + target.strength if (target.owner == self.game_map.my_id and (target.move == -1 or target.move == STILL)) else 0
        ...
        if future_strength <= 255 + strength_buffer:
            ...
            self.game_map.make_move(self, direction)
            return True
        
        # Ok, what can we do?
        # Case 1: The cell we are moving to is staying still. Can we get it to move to our target and chain to our destination?
        
        if target.owner == self.game_map.my_id and (target.move == -1 or target.move == STILL):
            if self.strength + sum(x.strength for x in target.moving_here) <= 255 + strength_buffer:
                # Ok, let's try to move this cell to the same destination to chain.
                success = target.move_to_target_old(destination, False) # Infinite loop encountered here, why?
                if success:
                    self.game_map.make_move(self, direction)
                    return True
                ...
                if target.moving_here != []:
                    for secondary_target in target.moving_here:
                        success = target.move_to_target(secondary_target.target, False)
                        if success:
                            self.game_map.make_move(self, direction)
                            return True
                ...
                for n in self.neighbors():
                    ...
                    neighbor_targets.append((n, neighbor_strength + target.strength, n.owner == self.game_map.my_id))
                ...
                if neighbor_targets[0][1] < 255 + strength_buffer:
                    self.game_map.make_move(self, direction)
                    target.move_to_target(neighbor_targets[0][0], False)
                    return True        
            
            
                
                
    def move_to_target_old(self, destination, through_friendly):
        ...
        if possible_target.moving_here != []:
        ...
        if possible_target.owner == self.game_map.my_id and (possible_target.move == -1 or possible_target.move == STILL):
            if self.strength + sum(x.strength for x in possible_target.moving_here) <= 255 + strength_buffer:
                # Ok, let's try to move this cell to the same destination to chain.
                success = possible_target.move_to_target_old(destination, False)
                if success:
                    self.game_map.make_move(self, possible_moves[0][0])
                    return True
                # Is there anywhere else we can move this cell?
                if possible_target.moving_here != []:
                    for secondary_target in possible_target.moving_here:
                        success = possible_target.move_to_target_old(secondary_target.target, False)
                        if success:
                            self.game_map.make_move(self, possible_moves[0][0])
                            return True
                # Ok, Is there another square we can make a move into??
                ...
                if neighbor_targets[0][1] < 255 + strength_buffer:
                    self.game_map.make_move(self, possible_moves[0][0])
                    possible_target.move_to_target_old(neighbor_targets[0][0], False)
                    return True
...
class Maps():
    ...
    def update(self):
        ...
        self.update_influence_production_map()
        self.update_influence_prod_over_str_map()
        
        self.update_heuristic_map()
        
        self.create_djikstra()
    ...
    def update_border_map(self):
        ...
        # Create a inner_border_map
        self.inner_border_map = numpy.zeros((self.width, self.height))
        self.inner_border_map = spread_n(self.border_map, 1)
        self.inner_border_map = numpy.multiply(self.inner_border_map, self.is_owner_map[self.game_map.my_id])
    ...
    def update_influence_production_map(self):
        # Lots of tweaking to do...
        # Start with a basic production map     
        ...
        self_prod_map = numpy.multiply(numpy.multiply(self.production_map, self.is_owner_map[self.game_map.my_id]), production_self_factor)
        neutral_prod_map = numpy.multiply(numpy.multiply(self.production_map, self.is_owner_map[0]), production_neutral_factor) 
        enemy_prod_map = numpy.multiply(numpy.multiply(self.production_map, self.is_enemy_map), production_enemy_factor)
        #modified_production_map = numpy.sum(numpy.sum(self_prod_map, neutral_prod_map), enemy_prod_map)
        modified_production_map = self_prod_map + neutral_prod_map + enemy_prod_map
        ...
        self_prod_map = numpy.multiply(numpy.multiply(self.production_map, self.is_owner_map[self.game_map.my_id]), late_game_production_self_factor)
        neutral_prod_map = numpy.multiply(numpy.multiply(self.production_map, self.is_owner_map[0]), late_game_production_neutral_factor) 
        enemy_prod_map = numpy.multiply(numpy.multiply(self.production_map, self.is_enemy_map), late_game_production_enemy_factor)
        #modified_production_map = numpy.sum(numpy.sum(self_prod_map, neutral_prod_map), enemy_prod_map)
        modified_production_map = self_prod_map + neutral_prod_map + enemy_prod_map
    ...
    def update_influence_prod_over_str_map(self):
        ...
        # Calculate the production / str maps.
        prod_str_map = numpy.divide(self.production_map, numpy.maximum(1, self.strength_map))
        scaled_prod_str_map = numpy.multiply(prod_str_map, self.is_owner_map[self.game_map.my_id]) * prod_over_str_self_factor + numpy.multiply(prod_str_map, self.is_owner_map[0]) * prod_over_str_neutral_factor + numpy.multiply(prod_str_map, self.is_enemy_map) * prod_over_str_enemy_factor
        late_game_scaled_prod_str_map = numpy.multiply(prod_str_map, self.is_owner_map[self.game_map.my_id]) * late_game_prod_over_str_self_factor + numpy.multiply(prod_str_map, self.is_owner_map[0]) * late_game_prod_over_str_neutral_factor + numpy.multiply(prod_str_map, self.is_enemy_map) * late_game_prod_over_str_enemy_factor
        # Diffuse the production map so that high strength areas might be targeted.
        self.influence_prod_over_str_map = spread_n(scaled_prod_str_map, production_decay, production_influence_max_distance)
        self.late_game_influence_prod_over_str_map = spread_n(late_game_scaled_prod_str_map, production_decay, production_influence_max_distance)
    
        self.influence_prod_over_str_map -= numpy.multiply(self.influence_prod_over_str_map, self.is_owner_map[self.game_map.my_id])

    def update_heuristic_map(self):
        self.heuristic_map = numpy.zeros((self.width, self.height))
        
        if self.game_map.phase <= 1:
        
            self.heuristic_map += production_square_influence_factor * self.rebase_map(self.production_map / numpy.maximum(self.strength_map, 1))
            self.heuristic_map += self.rebase_map(self.influence_production_map) * production_influence_factor
            self.heuristic_map += self.rebase_map(self.influence_prod_over_str_map) * prod_over_str_influence_factor
            #self.heuristic_map += numpy.multiply(numpy.divide(self.production_map, numpy.maximum(self.strength_map, 1)), self.is_owner_map[0]) * prod_over_str_neutral_factor
            #self.heuristic_map += numpy.multiply(numpy.divide(self.production_map, numpy.maximum(self.strength_map, 1)), self.is_owner_map[self.game_map.my_id]) * prod_over_str_self_factor
            #self.heuristic_map += numpy.multiply(numpy.divide(self.production_map, numpy.maximum(self.strength_map, 1)), self.is_enemy_map) * prod_over_str_enemy_factor
            
            #self.heuristic_map += self.rebase_map(numpy.multiply(self.production_map, self.is_owner_map[0])) * production_neutral_factor
            #self.heuristic_map += self.rebase_map(numpy.multiply(self.production_map, self.is_owner_map[self.game_map.my_id])) * production_self_factor
            #self.heuristic_map += self.rebase_map(numpy.multiply(self.production_map, self.is_enemy_map)) * production_enemy_factor
        
            self.heuristic_map += self.rebase_map(numpy.multiply(self.strength_map, self.is_enemy_map)) * enemy_strength_0_influence_factor
            self.heuristic_map += self.rebase_map(self.influence_enemy_strength_map[1]) * enemy_strength_1_influence_factor
            self.heuristic_map += self.rebase_map(self.influence_enemy_strength_map[2]) * enemy_strength_2_influence_factor
            self.heuristic_map += self.rebase_map(self.influence_enemy_strength_map[3]) * enemy_strength_3_influence_factor
    
            self.heuristic_map += self.rebase_map(self.influence_enemy_territory_map[1]) * enemy_territory_1
            self.heuristic_map += self.rebase_map(self.influence_enemy_territory_map[2]) * enemy_territory_2
            self.heuristic_map += self.rebase_map(self.influence_enemy_territory_map[3]) * enemy_territory_3
                
        else:
           
            #self.heuristic_map += numpy.multiply(numpy.divide(self.production_map, numpy.maximum(self.strength_map, 1)), self.is_owner_map[0]) * late_game_prod_over_str_neutral_factor
            #self.heuristic_map += numpy.multiply(numpy.divide(self.production_map, numpy.maximum(self.strength_map, 1)), self.is_owner_map[self.game_map.my_id]) * late_game_prod_over_str_self_factor
            #self.heuristic_map += numpy.multiply(numpy.divide(self.production_map, numpy.maximum(self.strength_map, 1)), self.is_enemy_map) * late_game_prod_over_str_enemy_factor
            
            #self.heuristic_map += numpy.multiply(self.production_map, self.is_owner_map[0]) * late_game_production_neutral_factor
            #self.heuristic_map += numpy.multiply(self.production_map, self.is_owner_map[self.game_map.my_id]) * late_game_production_self_factor
            #self.heuristic_map += numpy.multiply(self.production_map, self.is_enemy_map) * late_game_production_enemy_factor
            self.heuristic_map += late_game_production_square_influence_factor * self.rebase_map(self.production_map / numpy.maximum(self.strength_map, 1))
            self.heuristic_map += self.rebase_map(self.late_game_influence_production_map) * late_game_production_influence_factor
            self.heuristic_map += self.rebase_map(self.late_game_influence_prod_over_str_map) * late_game_prod_over_str_influence_factor
            
            self.heuristic_map += numpy.multiply(self.rebase_map(numpy.multiply(self.strength_map, self.is_enemy_map)), late_game_enemy_strength_0_influence_factor)
            self.heuristic_map += numpy.multiply(self.rebase_map(self.influence_enemy_strength_map[1]), late_game_enemy_strength_1_influence_factor)
            self.heuristic_map += numpy.multiply(self.rebase_map(self.influence_enemy_strength_map[2]), late_game_enemy_strength_2_influence_factor)
            self.heuristic_map += numpy.multiply(self.rebase_map(self.influence_enemy_strength_map[3]), late_game_enemy_strength_3_influence_factor)
            self.heuristic_map += numpy.multiply(self.rebase_map(self.influence_enemy_territory_map[1]), late_game_enemy_territory_1)
            self.heuristic_map += numpy.multiply(self.rebase_map(self.influence_enemy_territory_map[2]), late_game_enemy_territory_2)
            self.heuristic_map += numpy.multiply(self.rebase_map(self.influence_enemy_territory_map[3]), late_game_enemy_territory_3)  
    ...
    def create_djikstra(self):
        # Cell [x, y] corresponds to x * self.width + y in the grid below.
        #self.dj_graph = numpy.ones((self.game_map.width * self.game_map.height, self.game_map.width * self.game_map.height)) * 999
        self.dj_graph = scipy.sparse.dok_matrix((self.game_map.width * self.game_map.height, self.game_map.width * self.game_map.height))
        # Create the distance graph


        for x in range(self.game_map.width):
            for y in range(self.game_map.height):
                coord = x * self.game_map.height + y

                self.dj_graph[coord, ((x + 1) % self.game_map.width) * self.game_map.height + ((y + 0) % self.game_map.height)] = self.get_cost(((x + 1) % self.game_map.width) * self.game_map.height + ((y + 0) % self.game_map.height))
                self.dj_graph[coord, ((x - 1) % self.game_map.width) * self.game_map.height + ((y + 0) % self.game_map.height)] = self.get_cost(((x - 1) % self.game_map.width) * self.game_map.height + ((y + 0) % self.game_map.height))
                self.dj_graph[coord, ((x + 0) % self.game_map.width) * self.game_map.height + ((y + 1) % self.game_map.height)] = self.get_cost(((x + 0) % self.game_map.width) * self.game_map.height + ((y + 1) % self.game_map.height))
                self.dj_graph[coord, ((x + 0) % self.game_map.width) * self.game_map.height + ((y - 1) % self.game_map.height)] = self.get_cost(((x + 0) % self.game_map.width) * self.game_map.height + ((y - 1) % self.game_map.height))
        
        #self.dj_path, self.dj_route = scipy.sparse.csgraph.dijkstra(self.dj_graph, return_predecessors = True)
                         
    def get_cost(self, cellnum):
        
        x = cellnum // self.game_map.height
        y = cellnum % self.game_map.height
        ...
        if self.game_map.maps[self.game_map.frame].is_owner_map[self.game_map.my_id, x, y] == 1:
            return self.game_map.maps[self.game_map.frame].production_map[x, y] + 1
        # Otherwise, the cost to move through this square is equal to the strength of the cell
        return self.game_map.maps[self.game_map.frame].strength_map[x, y]    

    def get_index(self, x, y):
        # Get the index of the point x, y
...
def heuristic(cell, source = None):
    ...
    if cell.owner == 0 and game_map.turns_left * 0.75 < cell.strength / cell.production:
        return 0
    ...
    if cell.owner == 0 and cell.strength / cell.production > 75:
        return 0
...
def first_turns_heuristic():
    ...
    for square in game_map:
        if square.is_npc_border():
            ...
            if game_map.maps[game_map.frame].influence_enemy_territory_map[3, square.x, square.y] > 0:
                game_map.phase = 1
    ...
    #max_cells_out = 5
    #for c in range(1, max_cells_out + 1):
        ...
        if square.moving_here == []:
            if square.strength > (square.production * early_game_buildup_multiplier):
                ...
                if game_map.get_distance(square, border_squares[0][0]) > 2:
                    square.move_to_target(border_squares[0][0], True, True)
