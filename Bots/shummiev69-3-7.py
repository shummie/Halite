...
class Game:
    ...
    def get_moves_attack(self):
        ...
        self.safe_to_move = np.zeros((self.w, self.h))
        ...
        for sq in combat_squares_1:
            if (sq.strength > sq.production) and (sq.move == -1):
                if sq.strength > 20 and sq.is_isolated():
                    ...
                    if enemy_count > 1:
                        ...
                        for (dx, dy) in diagonals:
                            ...
                            if dsq.owner == self.my_id and dsq.is_isolated() and (dsq.move == 4 or dsq.move == -1):
                                should_still = False
                                break
                    if should_still and (self.safe_to_move[sq.x, sq.y] == 0 or self.safe_to_move[sq.x, sq.y] > sq.strength):
                ...
                neutral_targets = []
                for n in sq.neighbors:
                    if self.enemy_strength_map[2, n.x, n.y] == 0 or (self.safe_to_move[n.x, n.y] == 0 or self.safe_to_move[n.x, n.y] > sq.strength):
                        if n.owner == 0 and n.strength == 0:
                            targets.append(n)
                        elif n.owner == 0 and n.strength > 0:
                            neutral_targets.append(n)
                        elif n.owner == self.my_id:
                            if combat_distance_matrix[n.x, n.y] < combat_distance_matrix[sq.x, sq.y]:
                ...
                if not success:
                    for t in alt_targets:
                        ...
                        if success:
                            ...
                            break
                if not success:
                    if self.safe_to_move[sq.x, sq.y] == 0 or self.safe_to_move[sq.x, sq.y] > sq.strength:
                        success = True
                        self.make_move(sq, STILL)
                        self.mark_unsafe(sq, sq)
                if not success:
            ...
            if ((sq.strength > (sq.production * (self.buildup_multiplier[sq.x, sq.y] + 2))) or sq.strength > 250) and sq.move == -1 and sq.moving_here == []:
                ...
                for n in sq.neighbors:
                    if self.is_owned_map[n.x, n.y]:
                        if combat_distance_matrix[n.x, n.y] == current_distance - 1 and (self.safe_to_move[n.x, n.y] == 0 or self.safe_to_move[n.x, n.y] > sq.strength):
    ...
    def get_moves_breakthrough(self):
        ...
        potential_squares_indices = np.transpose(np.nonzero((self.border_map - self.combat_zone_map) * (self.enemy_strength_map[1] > 0) * (self.safe_to_move == 0)))
    ...
    def mark_unsafe(self, source, sq):
        if source.strength < min(game.frame // 2, 25):
            # This square isn't valuable enough to care about overkill
            return
        future_str = sum(x.strength for x in sq.moving_here) - source.strength
        for n in sq.neighbors:
            if n.owner == 0 and n.strength > 0:
                continue
            elif n.owner == self.my_id:
                continue
            elif n.owner != self.my_id and n.owner != 0 and n.strength > 25 and (n.strength - source.strength) < -25:
                for m in sq.neighbors:
                    self.safe_to_move[m.x, m.y] = m.strength - source.strength - future_str
                for m in n.neighbors:
                    self.safe_to_move[m.x, m.y] = m.strength - source.strength - future_str
            elif n.owner == 0 and n.strength == 0:
                for m in n.neighbors:
                    if m.owner != 0 and m.owner != self.my_id and m.strength > 25 and (m.strength - source.strength) < -25:
                        for r in n.neighbors:
                            self.safe_to_move[r.x, r.y] = r.strength - source.strength - future_str
