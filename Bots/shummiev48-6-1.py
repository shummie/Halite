...
class Game:
    ...
    def get_moves_breakthrough(self):
        ...
        for square in potential_squares:
            if self.own_strength_map[4, square.x, square.y] > 300 and (self.own_strength_map[4, square.x, square.y] > 1.5 * self.enemy_strength_map[4, square.x, square.y]):
                self.attack_cell(square, 1)
    ...
    def get_moves_other(self):
        ...
        for square in idle_squares:
            if square.strength > 240:
                ...
                if self.distance_from_border[square.x, square.y] == 1:
                    targets = [x for x in square.neighbors if (x.owner == 0 and self.production_map[x.x, x.y] > 0)]
            ...
            if square.strength > square.production * self.buildup_multiplier[square.x, square.y] and square.move == -1 and square.moving_here == []:
                if self.percent_owned > 0.65:
                    ...
                    value_map = (self.value_map + self.distance_map_no_decay[square.x, square.y] ** 1.5) * self.border_map
                    ...
                    value_map += self.distance_map_no_decay[square.x, square.y] ** 1.2 * self.combat_zone_map
