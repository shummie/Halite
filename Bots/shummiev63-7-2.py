...
class Game:
    ...
    def update_value_production_map(self):
        ...
        for g in global_targets:
            if self.base_value_map[g.x, g.y] > 0.1:
                ...
                self.global_border_map[tx, ty] += ((self.production_map_01[g.x, g.y] * 1.3) / self.strength_map_1[g.x, g.y]) / (self.dij_recov_distance_map[g.x, g.y, tx, ty])
...
#                return
    ...
    def attack_cell(self, target, max_cells_out, min_cells_out=1):
        ...
        while cells_out <= max_cells_out:
            ...
            production_cost = np.sum(self.production_map * (target_distance_matrix_production > 0))

            if available_strength + available_production > target.strength + production_cost + 0:
