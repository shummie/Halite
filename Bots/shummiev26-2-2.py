...
class GameMap:
    ...
    def get_best_moves(self):
        ...
        for square in cells_to_consider_moving:
            if self.inner_border_map[square.x, square.y]:
                combat_squares = []
                for n in square.neighbors():
                    if n.strength == 0 and n.owner == 0:
                        combat_squares.append(n)
                if combat_squares != []:
                    combat_squares.sort(key = lambda x: self.influence_enemy_strength_map[1, x.x, x.y])
            ...
            if square.strength > (square.production * buildup_multiplier) and len(square.moving_here) == 0:
                ...
                value_map += numpy.multiply(self.recover_map[0] + turns_to_capture + self.dij_prod_distance_map[square.x, square.y, :, :] ** 2, self.is_neutral_map - self.combat_zone_map)
                value_map += numpy.multiply(self.dij_prod_distance_map[square.x, square.y, :, :] ** 2 + 10, self.combat_zone_map)
