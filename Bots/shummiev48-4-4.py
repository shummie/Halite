...
class Game:
    ...
    def update_value_maps(self):
        ...
        for g in global_targets:
            ...
            self.global_border_map[tx, ty] += base_value_map[g.x, g.y] / self.dij_recov_distance_map[g.x, g.y, tx, ty]

        self.value_map = 1 / np.maximum(base_value_map + self.global_border_map * 1, 0.001)
    ...
    def attack_borders(self):
        ...
        #         continue
            ...
            if square.strength == 0 and self.combat_zone_map[square.x, square.y] == 0:
                self.value_map[square.x, square.y] = 9999  # We've captured this cell. 
    ...
    def move_inner_squares(self):
        ...
        for s in idle_squares:
            ...
            if s.strength > (s.production * self.buildup) and s.moving_here == []:
                ...
                if self.combat_zone_map[t.x, t.y] == 0 and s.strength > t.strength:
                    self.value_map[t.x, t.y] = 1 / self.global_border_map[t.x, t.y]
        ...
        while cells_out <= max_cells_out:
            ...
            if (available_strength + available_production) > (target.strength + 0):
                ...
                if needed_strength_from_movers > 0:
                    ...
                    moving_cells.sort(key=lambda x: x.strength)
                    # There are probably ways to do this more efficiently, for now just start with the highest strength cell (Note, not true right now)
