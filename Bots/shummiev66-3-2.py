...
class Game:
    ...
    @timethis
    def update_future_maps(self):
        self.future_strength_map = (self.strength_map * self.is_owned_map)
        self.resolved_str_map = (self.future_strength_map - self.enemy_strength_map[2])
    ...
    def update_value_production_map(self):
        ...
        global_targets = [self.squares[c[0], c[1]] for c in np.transpose(np.nonzero(self.is_neutral_map - self.combat_zone_map))]
    ...
    def get_moves_attack(self):
        ...
        for sq in combat_squares:
            if (sq.strength > 0) and (combat_distance_matrix[sq.x, sq.y] == 1) and (sq.move == -1 or sq.move == STILL):
                targets = []
                alt_targets = []
                for n in sq.neighbors:
                ...
                if not success:
                    for t in targets:
                        ...
                        if success:
                            break
            # elif ((sq.strength > (sq.production * (self.buildup_multiplier[sq.x, sq.y] + self.distance_from_combat_zone[sq.x, sq.y]))) or sq.strength > 250) and (sq.parity == self.parity) and sq.move == -1 and sq.moving_here == []:
            elif ((sq.strength > (sq.production * (self.buildup_multiplier[sq.x, sq.y] + 2))) or sq.strength > 250) and (sq.parity == self.parity) and sq.move == -1 and sq.moving_here == []:
                if combat_distance_matrix[sq.x, sq.y] == 2:
                    # Check for potential overkill damage.
                    possible_moves = []
                    for n in sq.neighbors:
                        if self.is_owned_map[n.x, n.y]:
                            if combat_distance_matrix[n.x, n.y] == 1:
                                if self.future_strength_map[n.x, n.y] + sq.strength <= self.str_cap:
                                    possible_moves.append(n)
                    if len(possible_moves) > 0:
                        possible_moves.sort(key=lambda sq: self.enemy_strength_map[4, sq.x, sq.y], reverse=True)
                        possible_moves.sort(key=lambda sq: self.resolved_str_map[sq.x, sq.y], reverse=True)
                        self.move_square_to_target(sq, possible_moves[0], True)
                else:
                    self.move_towards_map_old(sq, combat_distance_matrix)
            else:
                if combat_distance_matrix[sq.x, sq.y] > 1:
    ...
    def get_moves_prepare_strength(self):
        ...
        if len(enemy_border_squares) > 0:
            combat_distance_matrix = self.flood_fill(enemy_border_squares, 4, True)
            combat_distance_matrix[combat_distance_matrix == -1] = 0
            combat_squares = [self.squares[c[0], c[1]] for c in np.transpose(np.nonzero(combat_distance_matrix))]

            for square in combat_squares:
                if (self.distance_from_border[square.x, square.y] > 1) and (square.strength > square.production * self.buildup_multiplier[square.x, square.y]) and square.move == -1 and square.moving_here == []:
                    self.move_towards_map_old(square, combat_distance_matrix)
                elif (square.strength >= 200) and (self.own_strength_map[2, square.x, square.y] >= 750) and (combat_distance_matrix[square.x, square.y] == 1):
                    # Attack
                    targets = []
        ...
        for sq in potential_squares:
            ...
            for n in sq.neighbors:
                if n.owner in enemy_targets:
                    ...
                    break
            if attack:
                if self.own_strength_map[4, sq.x, sq.y] > 750 and (self.own_strength_map[4, sq.x, sq.y] > 1.5 * self.enemy_strength_map[4, sq.x, sq.y]):
                    self.attack_cell(sq, 1)
    ...
    def make_move(self, square, direction):
        ...
        if direction == -1:  # Reset the square move
            if square.target is not None:
                tx, ty = square.target.x, square.target.y
                self.resolved_str_map[tx, ty] -= square.strength
                self.resolved_str_map[square.x, square.y] += square.strength
                self.future_strength_map[tx, ty] -= square.strength
                self.future_strength_map[square.x, square.y] += square.strength
    ...
    def move_square_to_target(self, source, destination, through_friendly):
        ...
        if source_distance == -1 or source_distance == 0:
            return False # We couldn't find a path to the destination or we're trying to move STILL
