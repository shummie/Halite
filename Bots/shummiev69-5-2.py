...
class Game:
            ...
            if (square.strength > 0) and (combat_distance_matrix[square.x, square.y] == 1) and (square.move == -1 or square.move == STILL):
                ...
                if not success:
                    for t in targets:
                        ...
                        if success:
                            break
            elif (square.move == -1 and combat_distance_matrix[square.x, square.y] == 2 and square.strength > 0):
                targets = []
                alt_targets = []
                for n in square.neighbors:
                    if n.owner == self.my_id:
                        if combat_distance_matrix[n.x, n.y] == 1 and n.target is None:
                            targets.append(n)
    ...
    def get_moves_breakthrough(self):
        ...
        potential_squares_indices = np.transpose(np.nonzero((self.border_map - self.combat_zone_map)))
        ...
        for sq in potential_squares:
            ...
            for n in sq.neighbors:
                if n.owner in enemy_targets:
                    ...
                    break
            if attack:
                if self.own_strength_map[4, sq.x, sq.y] > 200:  # and (self.own_strength_map[4, sq.x, sq.y] > 1.5 * self.enemy_strength_map[4, sq.x, sq.y]):
                    attack = False
                    for n in sq.neighbors:
