...
class Game:
    ...
    def get_pre_combat_production(self):
        ...
        potential_targets = [self.squares[c[0], c[1]] for c in potential_targets_indices if (1 / self.value_production_map[c[0], c[1]] < self.pre_combat_threshold)]
        if len(potential_targets) == 0:
            return

        potential_targets.sort(key=lambda sq: 1 / self.value_production_map[sq.x, sq.y])

        best_target_value = 1 / self.value_production_map[potential_targets[0].x, potential_targets[0].y]
        ...
        while len(potential_targets) > 0 and 1 / self.value_production_map[potential_targets[0].x, potential_targets[0].y] <= (best_target_value + 1):
