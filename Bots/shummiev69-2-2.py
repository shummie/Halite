            ...
            if (combat_distance_matrix[square.x, square.y] == 1):
                if (square.strength > 0) and (square.move == -1):
                        ...
                        if enemy_count > 1:
                            ...
                            for (dx, dy) in diagonals:
                                ...
                                if dsq.owner == self.my_id and dsq.is_isolated() and (dsq.move == 4 or dsq.move == -1):
                                    should_still = False
                                    break
                        if should_still and safe_to_move[square.x, square.y]:
                            self.make_move(square, STILL)
                            if self.distance_from_enemy[square.x, square.y] <= 2 and square.strength > 10:
                                for n in square.get_neighbors(2):
                                    if self.distance_from_enemy[n.x, n.y] <= 2 and self.enemy_strength_map[2, n.x, n.y] > 50:
                                        safe_to_move[n.x, n.y] = 0
                            continue
                    targets = []
                    alt_targets = []
                    inside_target = []
                    for n in square.neighbors:
                        if self.enemy_strength_map[2, n.x, n.y] == 0 or safe_to_move[n.x, n.y]:
                    ...
                    for t in targets:
                        ...
                        if success:
                            if self.distance_from_enemy[t.x, t.y] <= 2 and square.strength > 10:
                    ...
                    if not success:
                        for t in alt_targets:
                            ...
                            if success:
                                ...
                                break
                    if not success:
                        if safe_to_move[square.x, square.y]:
                            success = True
                            self.make_move(square, STILL)
                            if self.distance_from_enemy[square.x, square.y] <= 2 and square.strength > 10:
                                for n in square.get_neighbors(2):
            ...
            # elif ((square.strength > (square.production * (self.buildup_multiplier[square.x, square.y] + 2))) or square.strength > 250) and (square.parity == self.parity) and square.move == -1 and square.moving_here == []:
                ...
                for n in square.neighbors:
                    if self.is_owned_map[n.x, n.y]:
                        if combat_distance_matrix[n.x, n.y] == current_distance - 1 and safe_to_move[n.x, n.y]:
                ...
                if len(possible_moves) > 0:
                    ...
                    if self.distance_from_enemy[n.x, n.y] <= 2 and square.strength > 10:
