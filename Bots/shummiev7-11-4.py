...
production_influence_factor = 0.25
production_square_influence_factor = 2
prod_over_str_influence_factor = .25
prod_over_str_self_factor = 0
prod_over_str_neutral_factor = 1
prod_over_str_enemy_factor = 2
enemy_strength_0_influence_factor = 2
enemy_strength_1_influence_factor = 2
enemy_strength_2_influence_factor = .5
enemy_strength_3_influence_factor = 0.25      
border_distance_decay_factor = 2
border_target_percentile = 0.6
...
class GameMap:
    ...
    def create_influence_prod_over_str_map(self):
        ...
        # Diffuse the production map so that high strength areas might be targeted.
        self.influence_prod_over_str_map = spread(scaled_prod_str_map, production_decay)
    ...
    def attack_cell(self, target, cells_out = 1):
        ...
        if available_strength + available_production > target.strength:
            ...
            if needed_strength_from_movers > 0:
                ...
                if len(moving_cells) > 0 and moving_cells[0].strength > needed_strength_from_movers:
                    # Ok, we know the strongest can capture it, start from the smallest and go upwards until we find something that can't capture it.
                    #moving_cells.reverse() # This might be itnerfering w combat
                    for single_cell_moving in moving_cells:
                        if single_cell_moving.strength > needed_strength_from_movers:
                            single_cell_moving.move_to_target(target, False)
                            break
                else:
                    # We need more than 1 cell, let's just start with the highest and go down
