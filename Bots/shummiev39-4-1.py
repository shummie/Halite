...
class Game:
    ...
    def set_configs(self):
        ...
        # If this is a small crowded map, default to normal behavior
        if min(self.width, self.height) / self.starting_player_count < 5:
            self.small_map = True
        else:
            self.small_map = False
    ...
    def update_configs(self):
        ...
        self.buildup_multiplier = self.buildup_multiplier + (self.distance_from_border ** 0.25)
...
#            self.distance_from_combat_zone += (1-self.is_owned_map) * 999
    ...
    def update_recover_maps(self):
        ...
        self.recover_wtd_map = 1 / np.maximum(self.prod_over_str_wtd_map, 0.001)
    ...
    def early_game_value_map(self):
        ...
        self.early_game_map = (self.border_map - self.combat_zone_map) * (self.enemy_strength_map[2] == 0) * self.recover_wtd_map
        ...
        if self.global_max_square is None:
            ...
            # np.savetxt("maps.txt", self.prod_over_str_avg_map)
            recover_map = 1 / np.maximum(self.prod_over_str_wtd_map, 0.01)
            ...
            if self.small_map and path_length > min(self.width, self.height) / ((self.starting_player_count - 1) / 2):
                self.phase = 1
        ...
        self.early_game_map[self.early_game_map > self.total_avg_cost_to_global - 4] = 9999

    def update_value_production_map(self):

        self.value_production_map = (self.border_map - self.combat_zone_map) * (self.enemy_strength_map[2] == 0) * self.recover_wtd_map
    ...
    def get_moves_attack(self):
        ...
        for square in combat_squares:
            if (not self.small_map or (square.strength > square.production * self.buildup_multiplier[square.x, square.y])) and ((square.x + square.y) % 2 == self.frame % 2) and square.move == -1:
...
#                return
    ...
    def get_moves_production(self):
        ...
        remove_targets = potential_targets[int(len(potential_targets) * 0.75):]
    ...
    def attack_cell(self, target, max_cells_out, min_cells_out=1):
        ...
        while cells_out <= max_cells_out:
            ...
            if target.strength > 0 or target.production > 4:
                free_squares = self.is_owned_map * (self.move_map == -1) * (self.strength_map >= (self.buildup_multiplier - 1) * self.production_map)
            else:
