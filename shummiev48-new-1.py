...
class Game:
    ...
    def update_value_maps(self):
        ...
        for b in potential_targets:
            b_global_bonus = np.sum(np.divide(base_value_map, np.maximum(self.dij_recov_distance_map[b.x, b.y], 1)))
            logging.debug(b_global_bonus)
            # self.value_map[b.x, b.y] = 1 / (base_value_map[b.x, b.y]) - (b_global_bonus) * 2
            self.value_map[b.x, b.y] = 1 / (base_value_map[b.x, b.y] + b_global_bonus)
    ...
    def attack_borders(self):
        ...
        potential_targets.sort(key=lambda x: self.value_map[x.x, x.y])
    ...
    def move_inner_squares(self):
        ...
        for s in idle_squares:
            if s.strength > (s.production * self.buildup) and s.moving_here == []:
                ...
                value_map = self.value_map + self.distance_map_no_decay[s.x, s.y] * self.border_map
                # Adjust combat squares
                value_map[np.nonzero(self.combat_zone_map)] = 5
                value_map += self.distance_map_no_decay[s.x, s.y] * 0.8 * self.combat_zone_map
