...
class Game:
    ...
    def create_parity_maps(self):
        ...
        self.odd = np.zeros((self.w, self.h))
    ...
    def update_border_maps(self):
        ...
        self.distance_from_border = self.friendly_flood_fill_multiple_sources(border_squares, max(self.w, self.h))
        ...
        if self.starting_player_count > 1 and np.sum(self.combat_zone_map) >= 1:  # Breaks in single player mode otherwise.
            ...
            self.distance_from_combat_zone = self.friendly_flood_fill_multiple_sources(combat_squares, max(self.w, self.h))
    ...
    def get_moves_attack(self):
        ...
        for square in combat_squares:
            ...
            elif ((square.strength > (square.production * (self.buildup_multiplier[square.x, square.y] + 1))) or square.strength > 250) and square.move == -1 and square.moving_here == []:
    ...
    def get_moves_prepare_strength(self):
        ...
        if len(enemy_border_squares) > 0:
            ...
            for square in combat_squares:
                if (self.distance_from_combat_zone[square.x, square.y] > 3) and (square.strength > square.production * self.buildup_multiplier[square.x, square.y] + 5) and square.move == -1 and square.moving_here == []:
                    self.move_towards_map_old(square, combat_distance_matrix)
        ...
        for square in potential_squares:
            ...
            for n in square.neighbors:
                if n.owner in enemy_targets:
                    ...
                    break
            if attack:
                if self.own_strength_map[4, square.x, square.y] > 500 and (self.own_strength_map[4, square.x, square.y] > 1.5 * self.enemy_strength_map[4, square.x, square.y]):
                    self.attack_cell(square, 1)
        ...
        for square in idle_squares:
            ...
            if square.strength > square.production * self.buildup_multiplier[square.x, square.y] and square.move == -1 and square.moving_here == []:
                if np.sum(self.combat_zone_map) == 0:
                    ...
                    if self.distance_from_combat_zone[square.x, square.y] < 6:
                        continue
                    if self.enemy_strength_map[3, square.x, square.y] > 0:
                        self.make_move(square, STILL, None)
                    else:
    ...
    def find_nearest_combat_zone(self, square):
        current_distance = self.distance_from_combat_zone[square.x, square.y]
        targets = []
        # Todo, minor optimization by moving to lower production square if possible.
        for n in square.neighbors:
            if self.is_owned_map[n.x, n.y]:
...
#                return
    ...
    def friendly_flood_fill(self, source, max_distance):
        # Returns a np.array((self.w, self.h)) that contains the distance to the target by traversing through friendly owned cells only.
    ...
    def non_friendly_flood_fill_multiple_sources(self, sources, max_distance=999):
        # Returns a np.array((self.w, self.h)) that contains the distance to the target by traversing through non owned cells only.
        # q is a queue(list) of items (cell, distance). sources is a list that contains the source cells.
        q = sources
        distance_matrix = np.ones((self.w, self.h), dtype=int) * -1
        for source in q:
