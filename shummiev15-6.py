...
botname = "shummie v15-6"

production_decay = 0.4
production_influence_max_distance = 15
buildup_multiplier = 6
early_game_buildup_multiplier = 7
early_game_value_threshold = 0.93
strength_buffer = 0
border_distance_decay_factor = 1.3
border_target_percentile = .6
future_map_decay = 1

production_self_factor = 2
production_neutral_factor = 2
production_enemy_factor = 5
production_influence_factor = 4 # Sample values are around 50-80
production_square_influence_factor = 2
prod_over_str_influence_factor = 5 # Sample values are around 0.5 - 1.0
prod_over_str_self_factor = -1
prod_over_str_neutral_factor = 1.5
prod_over_str_enemy_factor = 5
enemy_strength_0_influence_factor = 1
enemy_strength_1_influence_factor = 3 # Sample values around 4-20
enemy_strength_2_influence_factor = 5 # Sample values around 2-40
enemy_strength_3_influence_factor = 0 # Sample values around 2-50
enemy_territory_1 = 4
enemy_territory_2 = 7
enemy_territory_3 = 0
...
late_game_production_neutral_factor = 1
late_game_production_enemy_factor = 2
late_game_production_influence_factor = 4 # Sample values are around 50-80
late_game_production_square_influence_factor = 3
late_game_prod_over_str_influence_factor = 4 # Sample values are around 0.5 - 1.0
late_game_prod_over_str_self_factor = 1
late_game_prod_over_str_neutral_factor = 1
late_game_prod_over_str_enemy_factor = 3
late_game_enemy_strength_0_influence_factor = 3
...
late_game_enemy_territory_2 = 1
late_game_enemy_territory_3 = 0
...
class GameMap:
    ...
    def get_configs(self):
        ...
        self.early_game_squares_out_search = int(early_game_squares_out_search_array[map_size_index, player_index])
    ...
    def get_best_moves(self):
        ...
        for square in itertools.chain.from_iterable(self.squares):
            if square.is_npc_border():
                if self.maps[self.frame].influence_enemy_territory_map[8, square.x, square.y] == 0:
                    production_squares.append((square, self.maps[self.frame].recover_map[5, square.x, square.y]))
                else: 
    ...
    def go_to_border(self, square):
        ...
        border_square_list = [x for x in itertools.chain.from_iterable(self.squares) if self.maps[self.frame].border_map[x.x, x.y]]
        border_square_list.sort(key = lambda x: heuristic(x) / self.get_distance(square, x)**border_distance_decay_factor, reverse = True)
        
        #if len(self.border_square_list) > 0:        
        return square.move_to_target(border_square_list[0], True)
    ...
    def find_nearest_enemy_direction(self, square):
        ...
        while not success and index < 4:
            if heuristic(dir_distance[index][3]) > 0:
                success = square.move_to_target(dir_distance[index][3], True)
            index += 1
        if not success:
            self.make_move(square, random.choice([NORTH, WEST]))
    ...
    def late_game_attack(self):
        ...
        for square in itertools.chain.from_iterable(self.squares):
            ...
            if square.is_npc_border():
                all_border_targets.append((square, self.maps[self.frame].heuristic_map[square.x, square.y]))
        ...
        for square in cells_to_consider_moving:
            if square.is_border():
                ...
                targets = [n for n in square.neighbors() if (n.owner != self.my_id and n.strength < square.strength)]
                if len(targets) > 0:
                    targets.sort(key = lambda x: self.maps[self.frame].heuristic_map[square.x, square.y], reverse = True)
                    success = False
                    index = 0
    ...
    def get_best_moves_late_game(self):
        ...
        for square in itertools.chain.from_iterable(self.squares):
            if square.is_npc_border():
                all_targets.append((square, self.maps[self.frame].heuristic_map[square.x, square.y]))
...
class Square:
    ...
    def is_npc_border(self):
        ...
        # Have we done this calculation already? It shouldn't change within a frame
        return game_map.maps[game_map.frame].border_map[self.x, self.y]

    def move_to_target(self, destination, through_friendly, attempt_dj = True):
        ...
        if attempt_dj:
            source_index = self.x * self.game_map.width + self.y
            dest_index = destination.x * self.game_map.width + destination.y
            target_index = self.game_map.maps[self.game_map.frame].dj_route[dest_index, source_index]
            #target_index = self.game_map.maps[self.game_map.frame].dj_route[source_index, dest_index]
            tx = target_index // self.game_map.width
            ty = target_index % self.game_map.width
            dx = tx - self.x
            dy = ty - self.y
            ...
            possible_moves = [(direction, True, 1, 1)]
        ...
        if possible_target.owner == self.game_map.my_id and (possible_target.move == -1 or possible_target.move == STILL):
            if self.strength + sum(x.strength for x in possible_target.moving_here) <= 255 + strength_buffer:
                ...
                success = possible_target.move_to_target(destination, False, False)
                if success:
                    self.game_map.make_move(self, possible_moves[0][0])
                    return True
                ...
                if possible_target.moving_here != None:
                    for secondary_target in possible_target.moving_here:
                        success = possible_target.move_to_target(secondary_target.target, False, False)
                        if success:
                            self.game_map.make_move(self, possible_moves[0][0])
                            return True
                ...
                if neighbor_targets[0][1] < 255 + strength_buffer:
                    self.game_map.make_move(self, possible_moves[0][0])
                    possible_target.move_to_target(neighbor_targets[0][0], False, False)
                    return True
...
class Maps():
    ...
    def create_djikstra(self):
        ...
        for x in range(self.game_map.width):
            for y in range(self.game_map.height):
                coord = x * self.game_map.width + y

                self.dj_graph[coord, ((x + 1) % self.game_map.width) * self.game_map.width + ((y + 0) % self.game_map.height)] = self.get_cost(((x + 1) % self.game_map.width) * self.game_map.width + ((y + 0) % self.game_map.height))
                self.dj_graph[coord, ((x - 1) % self.game_map.width) * self.game_map.width + ((y + 0) % self.game_map.height)] = self.get_cost(((x - 1) % self.game_map.width) * self.game_map.width + ((y + 0) % self.game_map.height))
                self.dj_graph[coord, ((x + 0) % self.game_map.width) * self.game_map.width + ((y + 1) % self.game_map.height)] = self.get_cost(((x + 0) % self.game_map.width) * self.game_map.width + ((y + 1) % self.game_map.height))
                self.dj_graph[coord, ((x + 0) % self.game_map.width) * self.game_map.width + ((y - 1) % self.game_map.height)] = self.get_cost(((x + 0) % self.game_map.width) * self.game_map.width + ((y - 1) % self.game_map.height))
    ...
    def get_cost(self, cellnum):
        ...
        x = cellnum // self.game_map.width
        y = cellnum % self.game_map.width
        ...
        if self.game_map.maps[self.game_map.frame].is_owner_map[self.game_map.my_id, x, y] == 1:
            return self.game_map.maps[self.game_map.frame].production_map[x, y]
...
def heuristic(cell, source = None):
    ...
    if cell.owner == 0 and cell.strength / max(cell.production, 0.0001) > game_map.turns_left * 0.65:
        return 0
    ...
    return game_map.maps[game_map.frame].heuristic_map[cell.x, cell.y]
...
def first_turns_heuristic():
    ...
    for square in game_map:
        if square.is_npc_border():
            border_squares.append((square, game_map.maps[game_map.frame].recover_map_smooth[game_map.early_game_squares_out_search, square.x, square.y]))
    ...
    desired_squares = math.ceil(len(border_squares) / 10)
    border_squares = border_squares[0:desired_squares]
    all_squares = copy.copy(border_squares)
    ...
    for c in range(1, max_cells_out + 1):
        for border in border_squares:
            find_cell = False
            if border[1] <= threshold:
                find_cell = game_map.attack_cell(border[0], c)
            if find_cell: 
    ...
    for square in cells_to_consider_moving:
        if square.strength > (square.production * early_game_buildup_multiplier):
            ...
            #else:
            if game_map.get_distance(square, all_squares[0][0]) > 2:
                square.move_to_target(all_squares[0][0], True)
...
def game_loop():
    ...
    #if game_map.phase == 0 and numpy.sum(game_map.maps[game_map.frame].is_owner_map[game_map.my_id]) < (10*(game_map.width * game_map.height)**.5) / ((game_map.starting_player_count**0.5) * 10):
    if game_map.phase == 0:
        #start = time.time()
        first_turns_heuristic()
        #end = time.time()
        #logging.debug("13.4.1 Frame: " + str(game_map.frame) + " : " + str(end - start))
    #elif numpy.sum(game_map.maps[game_map.frame].is_owner_map[0]) > (game_map.width * game_map.height) * 0.4:
    if game_map.phase == 1:
