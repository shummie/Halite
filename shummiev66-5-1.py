...
class Game:
    ...
    def get_all_moves(self):
        # This function handles movement for all squares based on a set of rules and values

        owned_squares = [self.squares[c[0], c[1]] for c in np.transpose(np.nonzero(self.is_owned_map))]
        owned_squares.sort(key=lambda s: s.strength, reverse=True)
        owned_squares.sort(key=lambda s: self.distance_from_border[s.x, s.y])
        combat_zone_squares = [self.squares[c[0], c[1]] for c in np.transpose(np.nonzero(self.combat_zone_map))]
        if len(combat_zone_squares) > 0:
            combat_distance_matrix = self.flood_fill(combat_zone_squares, self.combat_radius, True)

        for sq in owned_squares:
            # all squares within combat radius distance must be assigned to a combat zone
            if sq.move != -1:
                continue

            if self.distance_from_combat_zone[sq.x, sq.y] == 1:
                targets = []
                for n in sq.neighbors:
                    if n.owner == 0 and n.strength == 0 and len(n.moving_here) == 0:
                        targets.append(n)
                if len(targets) > 0:
                    targets.sort(key=lambda n: self.enemy_strength_map[3, n.x, n.y], reverse=True)
                    for t in targets:
                        self.move_square_to_target_simple(sq, t, False)
                else:
                    targets = []
                    alt_targets = []
                    for n in sq.neighbors:
                    ...
                    if not success:
                        for t in targets:
                            ...
                            if success:
                                break
            elif self.distance_from_combat_zone[sq.x, sq.y] <= self.combat_radius:
                if ((sq.strength > (sq.production * (self.buildup_multiplier[sq.x, sq.y] + 2))) or sq.strength > 250) and (sq.parity == self.parity) and sq.moving_here == []:
                    self.move_towards_map_old(sq, combat_distance_matrix)
                else:
                    if combat_distance_matrix[sq.x, sq.y] > 1:
                        self.make_move(sq, STILL, None)
            else:
                # Find a square to attack, or border to reinforce.
                if sq.strength > sq.production * self.buildup_multiplier[sq.x, sq.y] and sq.moving_here == []:
                    if self.percent_owned > 0.65:
                        self.find_nearest_combat_zone(sq)
                    else:
                        if np.sum(self.is_owned_map) > 120:
                            d_map = self.distance_map_no_decay[sq.x, sq.y]
                        else:
                            d_map = self.flood_fill_to_border([sq])
                            d_map[d_map == -1] = 9999
                        value_map = (self.value_production_map + d_map * 1) * self.border_map
                        value_map[np.nonzero(self.combat_zone_map)] = 0
                        # value_map += d_map[sq.x, sq.y] * 0.66 * self.combat_zone_map
                        # value_map -= self.controlled_production_influence_map[5, sq.x, sq.y] * 5 * self.combat_zone_map
                        ...
                        if self.distance_between(sq, target) > 14:
                            self.move_square_to_target_simple(sq, target, True)
                        elif self.distance_between(sq, target) > 1:
                            self.move_square_to_target(sq, target, True)
                        else:
