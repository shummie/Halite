...
class Game:
    ...
    def get_moves_attack(self):
        ...
        combat_squares.sort(key=lambda x: self.enemies_in_range[2, x.x, x.y], reverse=True)
        ...
        for square in combat_squares:
                ...
                if (square.strength > square.production) and (square.move == -1):
                    ...
                    if not success:
                        for t in inside_target:
                            if t.strength + square.strength <= self.str_cap:
                                success = self.move_square_to_target_simple(square, t, True)
                                if success:
                                    self.mark_unsafe(t)
                                    break
            ...
            # elif ((square.strength > (square.production * (self.buildup_multiplier[square.x, square.y] + 2))) or square.strength > 250) and (square.parity == self.parity) and square.move == -1 and square.moving_here == []:
                ...
                if len(possible_moves) > 0:
                    ...
                    self.mark_unsafe(possible_moves[0])
                else:
                    self.make_move(square, STILL)
                    self.mark_unsafe(square)
            else:
    ...
    def mark_unsafe(self, square):
        if self.distance_from_enemy[square.x, square.y] <= 3 and square.strength > 25:
            for n in square.neighbors:
                if n.owner == self.my_id or (n.owner == 0 and n.strength == 0) or (n.owner != self.my_id and n.owner != 0):
