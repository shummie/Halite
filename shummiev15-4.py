...
future_map_decay = 0.9

production_square_factor = 1
enemy_strength_0_influence_factor = 4
...
enemy_territory_1 = 3
enemy_territory_2 = 0
future_value_factor = 1

late_game_buildup_multiplier = 7
late_game_production_square_factor = 3
late_game_enemy_strength_0_influence_factor = 4
late_game_enemy_strength_1_influence_factor = 2 # Sample values around 4-20
late_game_enemy_strength_2_influence_factor = 0 # Sample values around 2-40
late_game_enemy_strength_3_influence_factor = 0 # Sample values around 2-50
late_game_enemy_territory_0 = 4
late_game_enemy_territory_1 = 2
late_game_enemy_territory_2 = 0
late_game_future_value_factor = 1
...
class GameMap:
    ...
    def update(self):
        ...
        self.maps.append(Maps(self))
        self.maps[self.frame].update()
        
            
        #if self.phase == 0 and numpy.sum(self.maps[self.frame].is_owner_map[self.my_id]) > (10*(self.width * self.height)**.5) / ((self.starting_player_count**0.5) * 15):
        #    self.phase = 1
        if self.phase == 1 and numpy.sum(self.maps[self.frame].is_owner_map[0]) < (self.width * self.height * 0.4):
            self.phase = 2
    ...
    def attempt_move(self, move_list):
        # move_list is a list of (square, direction) 
        return
    ...
    def attack_cell(self, target, max_cells_out = 1):
        ...
        while cells_out <= max_cells_out:
            ...
            available_strength = numpy.sum(numpy.multiply(numpy.multiply(self.maps[self.frame].strength_map, numpy.minimum(distance_matrix, 1)), available_squares))
            ...
            available_production = numpy.sum(numpy.multiply(numpy.multiply(self.maps[self.frame].production_map, distance_matrix), available_squares))
            ...
            if available_strength + available_production > target.strength:
                ...
                still_strength = numpy.sum(numpy.multiply(numpy.multiply(self.maps[self.frame].strength_map, numpy.minimum(distance_matrix, 1)), available_squares)) # Note this is the new distance map used for available_production
...
class Square:
    ...
    def move_to_target(self, destination, through_friendly, attack_if_possible = False):
        ...
        possible_moves.append((NORTH, self.game_map.maps[self.game_map.frame].owner_map[(self.x + 0) % self.game_map.width, (self.y - 1) % self.game_map.height] == self.game_map.my_id, dist_n if dist_n > 0 else 999, self.game_map.maps[self.game_map.frame].production_map[(self.x + 0) % self.game_map.width, (self.y - 1) % self.game_map.height]))
        possible_moves.append((SOUTH, self.game_map.maps[self.game_map.frame].owner_map[(self.x + 0) % self.game_map.width, (self.y + 1) % self.game_map.height] == self.game_map.my_id, dist_s if dist_s > 0 else 999, self.game_map.maps[self.game_map.frame].production_map[(self.x + 0) % self.game_map.width, (self.y + 1) % self.game_map.height]))
        possible_moves.append((EAST, self.game_map.maps[self.game_map.frame].owner_map[(self.x + 1) % self.game_map.width, (self.y + 0) % self.game_map.height] == self.game_map.my_id, dist_e if dist_e > 0 else 999, self.game_map.maps[self.game_map.frame].production_map[(self.x + 1) % self.game_map.width, (self.y + 0) % self.game_map.height]))
        possible_moves.append((WEST, self.game_map.maps[self.game_map.frame].owner_map[(self.x - 1) % self.game_map.width, (self.y + 0) % self.game_map.height] == self.game_map.my_id, dist_w if dist_w > 0 else 999, self.game_map.maps[self.game_map.frame].production_map[(self.x - 1) % self.game_map.width, (self.y + 0) % self.game_map.height]))

           
        # through friendly only
        if through_friendly:
            if not attack_if_possible:
                possible_moves = [x for x in possible_moves if x[1]]
            else:
                possible_moves = [x for x in possible_moves if x[1] or self.game_map.get_target(self, x[0]).strength < self.strength]
...
def spread_n(M, n, decay = 0, include_self = True):
    ...
    while distance <= n:
        ...
        for c in combos:
            spread_map += roll_xy(numpy.multiply(M, decay_factor), c[0], c[1])
        distance += 1
    return spread_map
...
class Maps():
    def __init__(self, game_map):
        ...
        #self.distance_map = self.create_distance_map()
        #self.distance_map_no_decay = self.create_distance_map(1)     
        self.distance_map = game_map.distance_map
        self.distance_map_no_decay = game_map.distance_map_no_decay
    ...
    def update(self):
        ...
        self.update_future_value_map()
        self.update_time_to_recover_map()
    ...
    def update_influence_enemy_maps(self):
        ...
        for x in range(10):
            ...
            scale_factor = numpy.sum(self.influence_enemy_strength_map[x])
            self.influence_enemy_strength_map[x] = numpy.divide(self.influence_enemy_strength_map[x], scale_factor) * self.width * self.height
    
    def update_time_to_recover_map(self):
        max_distance = 30
        self.recover_map = numpy.zeros((max_distance + 1, self.width, self.height))
        
        self.recover_map[0] = numpy.divide(self.strength_map, numpy.maximum(self.production_map, 0.3))
    ...
    def update_future_value_map(self):
        max_distance = 30
        self.future_value_map = numpy.zeros((max_distance + 1, self.width, self.height))
        
        self.future_value_map[0] = numpy.divide(self.production_map, numpy.maximum(self.strength_map, 0.3))
        self.future_value_map[0] = numpy.multiply(self.future_value_map[0], 1 - self.is_owner_map[self.game_map.my_id])
        scale_factor = numpy.sum(self.future_value_map[0])
        self.future_value_map[0] = numpy.divide(self.future_value_map[0], scale_factor) * self.width * self.height
        distance = 1
        while distance <= max_distance:
            dir_map = numpy.zeros((4, self.width, self.height))
            dir_map[0] = roll_xy(self.future_value_map[distance - 1], 0, 1)
            dir_map[1] = roll_xy(self.future_value_map[distance - 1], 0, -1)
            dir_map[2] = roll_xy(self.future_value_map[distance - 1], 1, 0)
            dir_map[3] = roll_xy(self.future_value_map[distance - 1], -1, 0)
        
            self.future_value_map[distance] = numpy.add(self.future_value_map[distance - 1] * future_map_decay , numpy.amax(dir_map, 0))
            scale_factor = numpy.sum(self.future_value_map[distance]) * self.width * self.height
            self.future_value_map[distance] = numpy.divide(self.future_value_map[distance], scale_factor)
            distance += 1
            
        self.future_value_map_smooth = numpy.zeros((max_distance + 1, self.width, self.height))
        
        # For enemy squares, smooth out the strengths.
        total_enemy_strength = max(numpy.sum(numpy.multiply(self.strength_map, self.is_enemy_map)), 1)
        total_enemy_squares = numpy.sum(self.is_enemy_map)
        average_enemy_strength = total_enemy_strength / total_enemy_squares
        
        average_enemy_strength_map = numpy.multiply(self.is_enemy_map, average_enemy_strength)
        new_strength_map = numpy.maximum(numpy.add(average_enemy_strength_map, numpy.multiply(self.is_owner_map[0], numpy.maximum(self.strength_map, 0.3))), 1)
        
        self.future_value_map_smooth[0] = numpy.divide(self.production_map, new_strength_map)
        self.future_value_map_smooth[0] = numpy.multiply(self.future_value_map_smooth[0], 1 - self.is_owner_map[self.game_map.my_id])
        scale_factor = numpy.sum(self.future_value_map_smooth[0])
        self.future_value_map_smooth[0] = numpy.divide(self.future_value_map_smooth[0], scale_factor) * self.width * self.height
        distance = 1
        
        while distance <= max_distance:
            dir_map = numpy.zeros((4, self.width, self.height))
            dir_map[0] = roll_xy(self.future_value_map_smooth[distance - 1], 0, 1)
            dir_map[1] = roll_xy(self.future_value_map_smooth[distance - 1], 0, -1)
            dir_map[2] = roll_xy(self.future_value_map_smooth[distance - 1], 1, 0)
            dir_map[3] = roll_xy(self.future_value_map_smooth[distance - 1], -1, 0)
        
            self.future_value_map_smooth[distance] = numpy.add(self.future_value_map_smooth[distance - 1] * future_map_decay, numpy.amax(dir_map, 0))
            scale_factor = numpy.sum(self.future_value_map_smooth[distance])
            self.future_value_map_smooth[distance] = numpy.divide(self.future_value_map_smooth[distance], scale_factor) * self.width * self.height
            distance += 1
...
class EarlyGameEvaluator(Evaluator):
    ...
    def update(self):
        self.value_map = numpy.copy(self.game_map.maps[self.game_map.frame].recover_map)
        
        self.strategy_value_map = numpy.zeros((self.game_map.width, self.game_map.height))
        lookback = 5
        
        for frame in range(max(0, self.game_map.frame - lookback), self.game_map.frame + 1):
            
            #self.strategy_value_map += self.game_map.maps[frame].future_value_map_smooth[int(self.game_map.width / 2) - 1]
            self.strategy_value_map += self.game_map.maps[frame].future_value_map_smooth[7]
            #self.strategy_value_map -= self.game_map.maps[frame].is_owner_map[self.game_map.my_id] * self.game_map.maps[frame].future_value_map_smooth[int(self.game_map.width / 2) - 1] * 0.5

        # rescale to 1
        rescale_factor = numpy.sum(self.strategy_value_map)
        self.strategy_value_map = (self.strategy_value_map / rescale_factor) * self.game_map.width * self.game_map.height        
            
    def update_old(self):
        self.value_map[0] = numpy.divide(self.game_map.maps[self.game_map.frame].production_map, numpy.maximum(self.game_map.maps[self.game_map.frame].strength_map, 0.3))
        self.value_map[0] = numpy.multiply(self.value_map[0], 1 - self.game_map.maps[self.game_map.frame].is_owner_map[self.game_map.my_id])        
        distance = 1
...
class MidGameEvaluator(Evaluator):
    ...
    def update(self):
        ...
        # Frames back = 5
        lookback = 5
        self.value_map += self.game_map.maps[self.game_map.frame].recover_map[7]

        # Start with the future value state 7 turns out but smoothed.
        for frame in range(max(0, self.game_map.frame - lookback), self.game_map.frame + 1):
            self.strategy_value_map += self.game_map.maps[frame].future_value_map_smooth[int(self.game_map.width / 2) - 1]
            self.strategy_value_map -= self.game_map.maps[frame].is_owner_map[self.game_map.my_id] * self.game_map.maps[frame].future_value_map_smooth[int(self.game_map.width / 2) - 1] * 0.5
            
            prod_over_str = numpy.divide(self.game_map.maps[frame].production_map, numpy.maximum(self.game_map.maps[frame].strength_map, 1))
            scale_factor = numpy.sum(prod_over_str) * self.game_map.width * self.game_map.height
            
            self.value_map += prod_over_str * scale_factor * production_square_factor
            
            # Add the influence production map
            #self.value_map += self.game_map.maps[frame].influence_production_map * production_influence_factor    
            #self.value_map += self.game_map.maps[frame].influence_prod_over_str_map * prod_over_str_influence_factor
            
            self.value_map += self.game_map.maps[frame].influence_enemy_strength_map[0] * enemy_strength_0_influence_factor
            self.value_map += self.game_map.maps[frame].influence_enemy_strength_map[1] * enemy_strength_1_influence_factor
            self.value_map += self.game_map.maps[frame].influence_enemy_strength_map[2] * enemy_strength_2_influence_factor
            self.value_map += self.game_map.maps[frame].influence_enemy_strength_map[3] * enemy_strength_3_influence_factor
            
            self.value_map += self.game_map.maps[frame].influence_enemy_territory_map[0] * enemy_territory_0
            self.value_map += self.game_map.maps[frame].influence_enemy_territory_map[1] * enemy_territory_1
            self.value_map += self.game_map.maps[frame].influence_enemy_territory_map[2] * enemy_territory_2

        # rescale to 1
        rescale_factor = numpy.sum(self.strategy_value_map)
        self.strategy_value_map = (self.strategy_value_map / rescale_factor) * self.game_map.width * self.game_map.height * future_value_factor
        


        # Initialize with the actual prod/str of each cell
        prod_over_str = numpy.divide(self.game_map.maps[self.game_map.frame].production_map, numpy.maximum(self.game_map.maps[self.game_map.frame].strength_map, 1))
        scale_factor = numpy.sum(prod_over_str) * self.game_map.width * self.game_map.height
        ...
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_strength_map[0] * enemy_strength_0_influence_factor
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_strength_map[1] * enemy_strength_1_influence_factor
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_strength_map[2] * enemy_strength_2_influence_factor
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_strength_map[3] * enemy_strength_3_influence_factor
        
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_territory_map[0] * enemy_territory_0
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_territory_map[1] * enemy_territory_1
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_territory_map[2] * enemy_territory_2        

        # The value of any cell we own is 0.
        self.value_map = numpy.multiply(self.value_map, 1 - self.game_map.maps[self.game_map.frame].is_owner_map[self.game_map.my_id])
        
        # If this cell is neutral AND bordered by hostile, let's not attack it unless it's a recent combat zone (Strength == 0)
        self.value_map = numpy.multiply(self.value_map, 1 - numpy.multiply(self.game_map.maps[self.game_map.frame].enemy_border_map, self.game_map.maps[self.game_map.frame].strength_map > 0))
...
class LateGameEvaluator(Evaluator):
    ...
    def update(self):
        ...
        # Start with the future value state 7 turns out but smoothed.
        for frame in range(max(0, self.game_map.frame - lookback), self.game_map.frame + 1):
            self.value_map += self.game_map.maps[frame].future_value_map_smooth[5]
            self.strategy_value_map += self.game_map.maps[frame].future_value_map_smooth[int(self.game_map.width / 2) - 1]
        ...
        self.value_map += prod_over_str * scale_factor * late_game_production_square_factor

        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_strength_map[0] * late_game_enemy_strength_0_influence_factor
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_strength_map[1] * late_game_enemy_strength_1_influence_factor
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_strength_map[2] * late_game_enemy_strength_2_influence_factor
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_strength_map[3] * late_game_enemy_strength_3_influence_factor
        
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_territory_map[0] * late_game_enemy_territory_0
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_territory_map[1] * late_game_enemy_territory_1
        self.value_map += self.game_map.maps[self.game_map.frame].influence_enemy_territory_map[2] * late_game_enemy_territory_2       

        self.value_map += self.game_map.maps[self.game_map.frame].future_value_map_smooth[7] * late_game_future_value_factor
        # The value of any cell we own is 0.
        self.value_map = numpy.multiply(self.value_map, 1 - self.game_map.maps[self.game_map.frame].is_owner_map[self.game_map.my_id])
        
        # If this cell is neutral AND bordered by hostile, let's not attack it.
        self.value_map = numpy.multiply(self.value_map, 1 - numpy.multiply(self.game_map.maps[self.game_map.frame].enemy_border_map, (self.game_map.maps[self.game_map.frame].strength_map > 0) * 1))
        #self.value_map = numpy.multiply(self.value_map, numpy.multiply(self.game_map.maps.strength_map != 0, 1 - numpy.multiply(self.game_map.maps.is_owner_map[0], self.game_map.maps.enemy_border_map)))
...
class EarlyTacticalAI(TacticalAI):
    def get_moves(self):
        ...
        for square in self.game_map:
            ...
            if self.game_map.maps[self.game_map.frame].border_map[square.x, square.y]:
                ...
                border_squares.append((square, self.game_map.evaluators[self.phase].value_map[5, square.x, square.y]))
                # For now, we will have the mid-game trigger occur here but we shoudl really have the game map evaluate this on its own.
                if self.game_map.maps[self.game_map.frame].influence_enemy_territory_map[3, square.x, square.y] > 0:
        ...
        cells_out = 7
        for c in range(1, cells_out + 1):
            for border in border_squares:
                find_cell = False
                if border[1] <= threshold:
                    find_cell = self.game_map.attack_cell(border[0], c)
                    if find_cell:
...
class MidTacticalAI(TacticalAI):
    ...
    def get_moves(self):
        ...
        all_targets = []
        production_targets = []
        #logging.debug(self.game_map.evaluators[self.phase].value_map)
        for square in self.game_map:
            if self.game_map.maps[self.game_map.frame].border_map[square.x, square.y] and self.game_map.evaluators[self.phase].value_map[square.x, square.y] > 0:
                if self.game_map.maps[self.game_map.frame].influence_enemy_territory_map[6, square.x, square.y] == 0:
                    # We're far enough away, let's use the early game heuristic function instead
                    production_targets.append((square, self.game_map.maps[self.game_map.frame].recover_map[7, square.x, square.y]))
                    all_targets.append((square, self.game_map.evaluators[self.phase].value_map[square.x, square.y]))
                else:
                    if (square.owner == 0 and square.production == 1 and square.strength > 40) or (square.production == 0 and square.strength < 1):
                        # A hack to ignore the really low production squares.
                        continue
                    all_targets.append((square, self.game_map.evaluators[self.phase].value_map[square.x, square.y]))
        #logging.debug(all_targets)
        #logging.debug(production_targets)
        # Are all cells equally valuable?
        # Let's keep the top X% of cells. 
        production_targets.sort(key = lambda x: x[1])
        ...
        for border in production_targets:
            ...
            if border[1] >= threshold:
                ...
                if find_cell:
                    all_targets.remove((border[0], self.game_map.evaluators[self.phase].value_map[border[0].x, border[0].y]))
...
class LateTacticalAI(TacticalAI):
    def get_moves(self):
        ...
        all_targets = []
        for square in self.game_map:
            if self.game_map.maps[self.game_map.frame].border_map[square.x, square.y] == 1:
                all_targets.append((square, self.game_map.evaluators[self.phase].value_map[square.x, square.y]))
...
class EarlyStrategyAI(StrategyAI):
    ...
    def get_moves(self):
        ...
        for square in self.game_map:
            ...
            if self.game_map.maps[self.game_map.frame].border_map[square.x, square.y]:
                #border_squares.append((square, self.game_map.evaluators[self.phase].value_map[self.game_map.early_game_squares_out_search, square.x, square.y]))
                border_squares.append((square, self.game_map.evaluators[self.phase].strategy_value_map[square.x, square.y]))
                # For now, we will have the mid-game trigger occur here but we shoudl really have the game map evaluate this on its own.
        border_squares.sort(key = lambda x: x[1], reverse = True)

        threshold = border_squares[0][1] * early_game_value_threshold # Only consider squares that are at least as valuable as x% of the most valuable cell.  
        border_squares = [b for b in border_squares if b[1] >= threshold]
        border_squares.sort(key = lambda x: self.game_map.evaluators[self.phase].strategy_value_map[x[0].x, x[0].y])
        
        for square in cells_to_consider_moving:
            if square.strength > (square.production * early_game_buildup_multiplier):
                # Get the time to recover map and add a distance cost.
                
                square_cost = self.game_map.maps[self.game_map.frame].recover_map[6] + self.game_map.distance_map[square.x, square.y, :, :] * 1.2
                square_cost = square_cost * self.game_map.maps[self.game_map.frame].border_map
                
                # Penalize by strength cost.
                square_cost += (self.game_map.maps[self.game_map.frame].is_owner_map[self.game_map.my_id] + self.game_map.maps[self.game_map.frame].is_enemy_map) * 999
                tx, ty = numpy.unravel_index(square_cost.argmin(), (self.game_map.width, self.game_map.height))
                #border_squares.sort(key = lambda x: self.game_map.get_distance(square, x[0]))
                target = self.game_map.squares[tx, ty]
                if len(square.moving_here) > 0:
                    continue
                if self.game_map.get_distance(square, target) > 4:
                    square.move_to_target(target, True)
                else:
                    if self.game_map.get_distance(square, target) == 1 and square.strength > target.strength:
                        square.move_to_target(target, False)
...
class MidStrategyAI(StrategyAI):
    def get_moves(self):
        ...
        cells_to_consider_moving.sort(key = lambda x: x.strength)        
        for square in cells_to_consider_moving:
            if square.is_border() == True:
                ...
                if len(targets) > 0:
                    targets.sort(key = lambda x: self.game_map.evaluators[self.phase].value_map[square.x, square.y], reverse = True)
                    square.move_to_target(targets[0], False)
                elif square.strength > (square.production * buildup_multiplier):
                    if len(square.moving_here) > 0:
                        continue
                    #targets = [n for n in square.neighbors()]
                    #targets.sort(key = lambda x: self.game_map.evaluators[self.phase].value_map[x.x, x.y])
                    self.game_map.find_nearest_enemy_direction(square)
                    #square.move_to_target(targets[0], True)
                ...
                #targets = [n for n in square.neighbors()]
                #targets.sort(key = lambda x: numpy.divide(self.game_map.evaluators[self.phase].strategy_value_map[x.x, x.y], self.game_map.distance_map[x.x, x.y, :, :]))
                self.game_map.find_nearest_enemy_direction(square)
                #square.move_to_target(targets[0], True)
                #self.go_to_border(square)
...
class LateStrategyAI(StrategyAI):
    def get_moves(self):
        ...
        for square in self.game_map:
            # Do we risk undoing a multi-move capture if we move a piece that's "STILL"?
            if square.owner == self.game_map.my_id and (square.move == -1 or square.move == STILL):
                cells_to_consider_moving.append(square)
