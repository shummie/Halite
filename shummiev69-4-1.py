...
class Game:
    ...
    def update_value_production_map(self):
        ...
        for g in global_targets:
            if self.base_value_map[g.x, g.y] > 0.02:
                ...
                gb_map = self.dij_recov_distance_map[g.x, g.y] * (self.border_map - self.combat_zone_map) * (self.enemy_strength_map[1] > 0)
                gb_map[gb_map == 0] = 9999
    ...
    def get_moves_attack(self):
        ...
        combat_squares_other = [s for s in combat_squares if combat_distance_matrix[s.x, s.y] > 2]
            ...
            if (sq.strength > sq.production) and (sq.move == -1):
                ...
                for n in sq.neighbors:
                    if n.owner == 0 and n.strength == 0:
                        if self.safe_to_move[n.x, n.y]:
                            safe_attack.append(n)
                        else:
                            if len(n.moving_here) > 0 and sum(x.strength for x in n.moving_here) + sq.strength <= self.str_cap:
                                safe_attack.append(n)
                            else:
                                unsafe_attack.append(n)
                    elif n.owner == 0 and n.strength > 0:
                        if sq.strength > n.strength and self.enemy_strength_map[1, n.x, n.y] > sq.strength:
                            neutral_targets.append(n)
                    elif n.owner == self.my_id:
                        inside_target.append(n)

                safe_attack.sort(key=lambda x: self.enemy_strength_map[2, x.x, x.y], reverse=True)
                safe_attack.sort(key=lambda x: self.distance_from_enemy[x.x, x.y])

                unsafe_attack.sort(key=lambda x: x.strength)
                unsafe_attack.sort(key=lambda x: self.distance_from_enemy[x.x, x.y])

                inside_target.sort(key=lambda x: x.strength)

                neutral_targets.sort(key=lambda x: x.strength)

                success = False
                for t in safe_attack:
                    success = self.move_square_to_target_simple(sq, t, False)
                    if success:
                        self.mark_unsafe(sq, t)
                        break
                if not success:
                    for t in neutral_targets:
                        ...
                        if success:
                            ...
                            break
                if not success:
                    if self.safe_to_move[sq.x, sq.y]:
                        success = True
                        self.make_move(sq, STILL)
                        self.mark_unsafe(sq, sq)
                if not success:
                    for t in inside_target:
                        if t.strength + sq.strength <= self.str_cap:
                            success = self.move_square_to_target_simple(sq, t, True)
                            if success:
                                self.mark_unsafe(sq, t)
                                break
                if not success:
                    for t in unsafe_attack:
                        success = self.move_square_to_target_simple(sq, t, False)
                        if success:
                            self.mark_unsafe(sq, t)
                            break
        ...
        for sq in combat_squares_2:
            if ((sq.strength > (sq.production * (self.buildup_multiplier[sq.x, sq.y] + 2))) or sq.strength > 250) and sq.move == -1 and sq.moving_here == []:
                ...
                if len(possible_moves) > 0:
                    ...
                    possible_moves.sort(key=lambda sq: self.distance_from_enemy[sq.x, sq.y])
                    success = False
                    for t in possible_moves:
                        success = self.move_square_to_target(sq, t, True)
                        if success:
                            self.mark_unsafe(sq, t)
                            break
                    if not success:
        ...
        for square in combat_squares_other:
            if ((square.strength > (square.production * (self.buildup_multiplier[square.x, square.y] + 2))) or square.strength > 250) and square.move == -1 and square.moving_here == []:
                ...
                if len(possible_moves) > 0:
                    ...
                    success = False
                    for t in possible_moves:
                        success = self.move_square_to_target(square, t, True)
                        if success:
                            break
                    if not success:
                        self.make_move(sq, STILL)
    ...
    def mark_unsafe(self, source, sq):
        ...
        for n in sq.neighbors:
            if n.owner == 0 and n.strength > 0:
                ...
                continue
            elif n.owner != self.my_id and n.owner != 0 and n.strength > threshold:  # and (source.strength - n.strength) > threshold:
                ...
                for m in n.neighbors:
                    if m.owner != 0 and m.owner != self.my_id and m.strength > threshold:  # and (source.strength - m.strength) > threshold:
        ...
        for sq in idle_squares:
            ...
            if sq.strength > sq.production * self.buildup_multiplier[sq.x, sq.y] and sq.move == -1 and sq.moving_here == []:
                if np.sum(self.combat_zone_map) == 0:
                    ...
                    if self.enemy_strength_map[3, sq.x, sq.y] > 0:
                        self.make_move(sq, STILL)
                    else:
