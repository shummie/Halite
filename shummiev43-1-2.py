...
class Game:
    ...
    def update_configs(self):
        ...
        if np.sum(self.combat_zone_map) > 3:
            self.production_cells_out = 3  # int(self.width / self.starting_player_count / 2.5)
    ...
    def update_value_production_map(self):
        ...
        self.value_production_map[border_square_closest_to_global.x, border_square_closest_to_global.y] = min(self.total_avg_cost_to_global - 3, avg_cost_to_global, self.value_production_map[border_square_closest_to_global.x, border_square_closest_to_global.y])
...
#                return
    ...
    def get_moves_breakthrough(self):
        ...
        for square in potential_squares:
            if self.own_strength_map[4, square.x, square.y] > 150 and (self.own_strength_map[4, square.x, square.y] > 1.5 * self.enemy_strength_map[4, square.x, square.y]):
                self.attack_cell(square, 1)
        ...
        for square in idle_squares:
            if square.strength > square.production * self.buildup_multiplier[square.x, square.y] and square.move == -1 and square.moving_here == []:
                if self.percent_owned > 0.65:
                    ...
                    value_map += self.distance_map_no_decay[square.x, square.y] * 0.3 * self.combat_zone_map
