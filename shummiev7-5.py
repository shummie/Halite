...
def find_nearest_enemy_direction(square):
    # This function is likely deprecated as of 7.5 but keeping the code here in case it proves useful
...
def go_to_border(square):
    ...
    for x in range(game_map.width):
        for y in range(game_map.height):
            ...
            scaled_value = game_map.influence_npc_production_map[y, x, 0] / distance
...
class GameMap:
    ...
    def create_production_influence_map(self):
        ...
        decay = 0.50
        self.influence_npc_production_map = numpy.zeros((self.height, self.width, self.starting_player_count + 1))
        for x in range(self.width):
            for y in range(self.height):
                ...
                if self.owner_map[y, x] == 0:
                    ...
                    str_value = self.strength_map[y, x]  # This will cause cells to avoid border cells...
                ...
                for c in combos:
                    ...
                    self.influence_npc_production_map[(y + c[1]) % self.height, (x + c[0]) % self.width, self.owner_map[y, x]] += value * decay_factor
