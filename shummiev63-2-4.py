...
class Game:
    ...
    def update_configs(self):
        self.buildup_multiplier = np.minimum(np.maximum(self.production_map, 5), 6)
        ...
        if np.sum(self.is_owned_map) > 6:
            self.buildup_multiplier += 1
        self.buildup_multiplier += (self.distance_from_border > 1) * 1
